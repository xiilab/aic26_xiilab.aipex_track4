...
# Selection needs per-epoch checkpoints, so the source tree wins; set RUNS_SRC to the local tree
R="${RUNS_ROOT_SEL:-$RUNS_SRC}"
export RUNS_ROOT="$R"
...
if [ "$LIST" = 1 ]; then
  ...
  printf '  %-14s %-14s %-42s %s\n' "model" "adopted" "heldout run (selection input)" "have"
  printf '  %-14s %-14s %-42s %s\n' "-------------" "-------------" "----------------------------------" "----"
  for m in "${MODELS[@]}"; do
    if hp="$(resolve_run "$R" "${HELDOUT_CAND[$m]}")"; then have="✓"; else have="✗"; fi
    printf '  %-14s %-14s %-42s %s\n' "$m" "${ADOPTED[$m]}" "$(basename "$hp")" "$have"
  done
  echo
...
for m in "${TARGETS[@]}"; do
  ...
  echo
  say "[02] $m  (adopted baseline ${ADOPTED[$m]})"
  "sel_$m"
done
echo
