...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if split in ("val", "test"):
            # Local modification: in-training evaluation is disabled, so the
            # training stage never reads evaluation data. Epoch selection is
            # done offline on a held-out bench with
            # train/encoders/beit3/beit3_tool.py eval.
            raise RuntimeError(
                "In-training evaluation for task 356 (split=%r) is disabled. "
                "Use train/encoders/beit3/beit3_tool.py eval for epoch selection "
                "(`--eval` mode goes through this path and is blocked as well)." % split)
        else:
            self.num_files = 75
            count = 0
        ...
        self.split = split
        # view_aug_mode: "both" (50:50 crop/nocrop), "global" (nocrop only), "local" (crop only)
        self.view_aug_mode = "both"
    ...
    def _get_image(self, image_path: str):
        ...
        if not os.path.exists(image_path):
            ...
            if "train" in parts:
                ...
                if train_idx + 3 < len(parts) and parts[train_idx + 1].startswith("imgs_"):
                    img_group = int(parts[train_idx + 1].split("_")[1])
                    part_name = f"Part {img_group // 8 + 1}"
        ...
        if self.view_aug_mode == "global":
            return self.nocrop(image)
        elif self.view_aug_mode == "local":
            return self.crop(image)
        else:  # "both" — 50:50 between crop and nocrop
    ...
    def _get_image_text_example(self, index: int, data: dict):
        ...
        # Multi-style captions: if the item carries a "captions" dict, sample one of them.
        if "captions" in item and isinstance(item["captions"], dict) and item["captions"]:
            import random as _rd
            caption = _rd.choice(list(item["captions"].values()))
        else:
...
def get_sentencepiece_model_for_beit3(args):
    ...
    return XLMRobertaTokenizer(vocab_file=args.sentencepiece_model)
...
def create_dataset_by_split(args, split, is_train=True):
    ...
    if hasattr(dataset, "view_aug_mode"):
        dataset.view_aug_mode = getattr(args, "view_aug_mode", "both")
        if is_train:
            print(f"[BaseDataset] view_aug_mode = {dataset.view_aug_mode}")
    if is_train:
...
def create_downstream_dataset(args, is_eval=False):
    if is_eval:
        ...
        data_loader_train = create_dataset_by_split(args, split="356", is_train=True)
        if args.task == "356":
            # Local modification: task 356 runs no in-training evaluation, so
            # there is no val loader. Epoch selection is done offline with
            # train/encoders/beit3/beit3_tool.py eval on a held-out bench.
