"""RSTP eva02_pre feats dump — EVA02-L-14 / merged2b_s4b_b131k zero-shot (open_clip).
Mirrors the backbone loading of encode_openclip_zs.py (eva02_pre member = EVA02-L-14 at 224px)
with RSTP as the data source. Saves {G, Q} feats rather than a [Q, G] score matrix, so the
combine step normalizes it the same way as the other members.

env: .venv_beit3eval  (open_clip 3.3.0)
run: CUDA_VISIBLE_DEVICES=6 $PY_beit3eval rstp_dump_eva02_pre.py
out: $ARTIFACTS/rstp_eva02_pre_feats.pt  {G, Q, gal_paths, Q_idx}
"""
import os, time
...
BATCH = 128
OUT = f"{T4}/rstp_eva02_pre_feats.pt"
dev = "cuda:0"
...
log(f"[rstp-eva02_pre] gallery={len(gal_paths)} queries={len(caps)}")
