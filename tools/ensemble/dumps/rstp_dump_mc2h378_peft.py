"""RSTP mc2h378_peft feats dump — MetaCLIP2 worldwide-huge-378 (ViT-H/14@378) DoRA ep02.
...
run: CUDA_VISIBLE_DEVICES=6 $PY_llm2clip rstp_dump_mc2h378_peft.py
out: $ARTIFACTS/rstp_mc2h378_peft_feats.pt  {G, Q, gal_paths, Q_idx}
"""
...
CKPT = "ep02"
BS = 256
OUT = f"{HERE}/rstp_mc2h378_peft_feats.pt"
dev = "cuda:0"
...
ckpt_dir = CKPT_DIR
print(f"[rstp-mc2h378_peft] MODEL={M.MODEL_NAME} IMG={M.IMAGE_SIZE} ckpt={CKPT} bs={BS}", flush=True)
...
print(f"[rstp-mc2h378_peft] gallery={len(gal_paths)} queries={len(caps)}", flush=True)
