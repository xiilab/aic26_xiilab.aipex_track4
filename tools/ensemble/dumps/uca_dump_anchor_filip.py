"""UCA v20 TTA-views dump (base/hflip/z080) — deployed_base anchor member.
Mirrors encode_anchor_filip.py (v20 l512, ep10) but encodes only the 3 views used by
build_base's deployed_base, with UCA as the data source. The v20 train module is imported and
its checkpoint loaded read-only.

env: .venv_llm2clip  (SigLIP2-L512 DoRA inference)
run: CUDA_VISIBLE_DEVICES=6 $PY_llm2clip uca_dump_anchor_filip.py
out: $ARTIFACTS/uca_anchor_filip_feats.pt  {img:{base,hflip,z080}, txt:{base}, gal_paths, Q_idx}
"""
...
BEST_CKPT = "ep10"
OUT = f"{T4}/uca_anchor_filip_feats.pt"
GALLERY_BATCH = 48
...
print(f"[uca-v20] gallery={len(gal_paths)} queries={len(caps)}", flush=True)
...
print(f"[uca-v20] ckpt={BEST_CKPT} IMG={S}", flush=True)
