"""UCA v28a TTA-views dump (base/hflip/z090) — anchor member.
...
env: .venv_llm2clip
run: CUDA_VISIBLE_DEVICES=6 $PY_llm2clip uca_dump_anchor_tcap.py
out: $ARTIFACTS/uca_anchor_tcap_feats.pt  {img:{base,hflip,z090}, txt:{base}, gal_paths, Q_idx}
"""
import os, sys
...
POOL = f"{T4}/uca_pool.pt"          # single source for gal_paths/caps ordering
OUT = f"{T4}/uca_anchor_tcap_feats.pt"
CKPT = os.environ.get("ENCODER_CKPT", f"{_REPO}/assets/model/encoder/anchor_tcap")   # RUN A ep09
VIEWS = ("base", "hflip", "z090")

dump_tta_views(POOL, OUT, ckpt=CKPT, views=VIEWS, tag="uca-v28a",
