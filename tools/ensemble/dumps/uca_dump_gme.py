"""UCA gme feats dump — GME-Qwen2-VL-2B-Instruct zero-shot (no adapter).
Uses get_text_embeddings / get_image_embeddings with the image token budget set to 64..256,
with UCA as the data source.

env: .venv_llm2clip
run: CUDA_VISIBLE_DEVICES=6 HF_HOME=$HF_CACHE $PY_llm2clip uca_dump_gme.py
out: $ARTIFACTS/uca_gme_feats.pt  {G, Q, gal_paths, Q_idx}
"""
import os, glob, time
...
print(f"[uca-gme] gallery={len(gal_paths)} queries={len(caps)}", flush=True)
