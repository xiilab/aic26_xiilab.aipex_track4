"""UCA metaclip2 feats dump — MetaCLIP2 worldwide-L/14 DoRA ep02.
...
run: CUDA_VISIBLE_DEVICES=6 $PY_llm2clip uca_dump_metaclip2.py
out: $ARTIFACTS/uca_metaclip2_feats.pt  {G, Q, gal_paths, Q_idx}
"""
...
CKPT = "ep02"
BS = 256
OUT = f"{HERE}/uca_metaclip2_feats.pt"
dev = "cuda:0"
...
print(f"[uca-metaclip2] MODEL={M.MODEL_NAME} IMG={M.IMAGE_SIZE} ckpt={CKPT}", flush=True)
...
print(f"[uca-metaclip2] gallery={len(gal_paths)} queries={len(caps)}", flush=True)
