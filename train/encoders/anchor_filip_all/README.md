...
```bash
python train/encoders/anchor_filip_all/train.py --gpu 0 --run-note anchor_filip_all
```
...
Outputs land in `assets/runs/anchor_filip_all/`.

## Next step

```bash
python train/encoders/anchor_filip_all/build_swa.py assets/runs/anchor_filip_all 8 10
python train/encoders/anchor_filip_all/deploy.py    assets/runs/anchor_filip_all
```

The range comes from [`anchor_filip_heldout/search_swa_range.py`](../anchor_filip_heldout/search_swa_range.py); omitting it uses the built-in `8 10`.

`deploy.py` writes to `assets/model_rep/encoder/anchor_filip/`.
