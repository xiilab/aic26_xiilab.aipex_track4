"""SigLIP2-L/16-512 + DoRA r16 + PI(64->128), FLAIR/TCAP, held-out excluded.

Same recipe and hyperparameters as anchor_tcap_all, with the held-out images removed.
...
# defined, built and gated by heldout_split_v1.py
# ensure the cache at startup, then drop every such image from the manifest
USE_HELDOUT_SPLIT       = True  # False trains on everything
HELDOUT_MODULE          = "heldout_split_v1"  # import target (same directory)
HELDOUT_REQUIRE_GATES   = True  # abort training if a gate fails

MODEL_NAME      = "google/siglip2-large-patch16-512"  # large-512 (hidden 1024, 24L, 1024 patch)
IMAGE_SIZE      = 512  # large-512 (hidden 1024, 24L, 1024 patch)
...
BATCH_SIZE      = 512  # 512 with GC on is ~130GB; with GC off even batch 128 OOMs at 512px
...
def _ensure_heldout_cache(verbose: bool = True) -> dict:
    ...
    if verbose:
        ...
        print(f"  [heldout] candidate pool {stats.get('pct_candidate_of_total')}% of train "
              f"(large near-duplicate components; pHash failures {stats.get('excluded_from_candidacy'):,} images "
              f"cannot be held out = sample bias, state this in the paper)")
        _bm = (f"boundary cos>={gates.get('g2_gate_thr')} pairs {gates.get('g2_cross_neardup_pairs')}"
               if gates.get("g2_sim_mode", "").startswith("embedding")
    ...
    if HELDOUT_REQUIRE_GATES and not gates.get("all_pass"):
        raise RuntimeError(
            ...
            f"  (HELDOUT_REQUIRE_GATES=False skips this, but a leaked dev set is worthless)")
    return stats
...
def train_single(physical_gpu: int,
    ...
    try:
        ...
        if USE_HELDOUT_SPLIT:
            print("\n[1b/5] Heldout split (near-duplicate components of the source data) ...")
            ...
            # the excluded count must equal the held-out image count (all style rows removed)
            ...
            # residual leak guard: zero held-out images among the training rows
...
def main():
    parser = argparse.ArgumentParser(
        description="SigLIP2-L/16-512 + DoRA + FLAIR/TCAP - held-out excluded (single GPU)")
