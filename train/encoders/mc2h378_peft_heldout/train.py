"""MetaCLIP2-H/14@378 + DoRA(v8/t16) + PI(77->128) + FLAIR + self-residual batching, held-out excluded.

Same recipe and hyperparameters as mc2h378_peft_all, with the held-out images removed.
The held-out list comes solely from heldout_split_v1.py.
...
# caption source CSV; train/gen/gen_manifest.py builds the manifest (12 preset styles, 1.01M images, 12.16M rows)
RECAP_CSV_PATH      = os.environ.get("RECAP_CSV", "assets/data/raw/recaption/train_msr_v2.csv")  # manifest source; only read for the staleness check
MANIFEST_TAG        = "v2"                        # msr caption generation; produced by gen_manifest.py
MANIFEST_PATH       = f"{MANIFEST_DIR}/pab_manifest_msr_{MANIFEST_TAG}.jsonl"

USE_UNIQUE_IMAGE_MODE = True

# defined, built and gated by heldout_split_v1.py (same asset as the SigLIP models)
USE_HELDOUT_SPLIT       = True  # False trains on everything
HELDOUT_MODULE          = "heldout_split_v1"
HELDOUT_REQUIRE_GATES   = True  # abort training if a gate fails
...
# images the un-adapted mc2-huge378 confuses are its own hard negatives, so co-locate them
# loss and model unchanged
V0031_NEIGHBOR_FILE  = os.environ.get("MC2H378_NEIGHBORS", "assets/data/manifest/mc2h378_selfresidual_neighbors.pt")
V0031_CO_LOCATE_FRAC = 0.40
V0031_NEIGHBORS_PER  = 1
V0031_ENABLED        = True

MODEL_NAME      = "facebook/metaclip-2-worldwide-huge-378"  # ViT-H/14 @378 (vision 1280, text 1024, proj 1024)
IMAGE_SIZE      = 378  # patch14: 378//14 = 27x27 = 729 patch
MAX_TEXT_LENGTH = 128  # MetaClip2 text is pretrained at 77 (CLIP-style); PI extends it to 128
                                                            # 14% of PAB queries exceed 77 tokens
POSITION_EMBED_NEW_SIZE = 128  # *logical* target length; no extra rows (PI is fractional)
...
NUM_PROBES               = 1  # K=1 disables multi-probe
...
ENABLE_PROBE_DIVERSITY_REG    = False
PROBE_DIVERSITY_REG_LAMBDA    = 2e-3  # unused at K=1
PROBE_DIVERSITY_WARMUP_EPOCHS = 2  # unused at K=1
...
SCHEDULE_HORIZON_EPOCHS = 30
BATCH_SIZE      = 512  # batch 128->512 (step 1/4). sqrt -> eff LR 1.4e-4
REF_BATCH       = 128  # batch the LR was tuned at
NUM_WORKERS     = 2  # /dev/shm is a shared 16GB, kept low for concurrent runs
...
FINEGRAIN_ENABLED     = False  # mutually exclusive with FLAIR
...
FINEGRAIN_AGG_RATIO   = 0.06  # k as a fraction of Lp (Lp=64 -> k=4)
FINEGRAIN_TARGET_LP   = 64  # FILIP late-interaction resolution (8x8); changing it is a different experiment
# startup fail-fast; blocks 36<->64 drift
# guard so a new IMAGE_SIZE/pool cannot fail while FINEGRAIN_ENABLED=False
...
FLAIR_WEIGHT_WARMUP_EPOCHS = 3  # linear warmup of lambda, protecting the randomly initialised pooler; 0 = none
...
FLAIR_PATCH_POOL   = 3  # huge-378 grid=27 -> ceil(27/3)=9 -> Lp=81
FLAIR_TARGET_LP    = 81  # 9x9; a 27 grid has no integer split into 64, so 81
FLAIR_STRIP_CLS    = True  # drop CLS (index 0) from the CLIP last_hidden_state, keeping patches only
FLAIR_GRID_DIV     = 14  # patch14 -> grid = IMAGE_SIZE // 14
...
if FLAIR_ENABLED:
    _flair_grid = IMAGE_SIZE // FLAIR_GRID_DIV  # patch14 grid
...
LR_BATCH_SCALING      = "sqrt"  # sqrt gives an effective LR of sqrt(512/128)*7e-5 = 1.4e-4; linear (2.8e-4) runs too hot
POSITION_PRETRAINED_FREEZE = True  # the 77 pretrained rows stay frozen; PI and DoRA adapt
POSITION_PRETRAINED_SIZE   = 77  # MetaClip2 text max_position_embeddings (CLIP-style learned pos emb)
# MetaClip2TextEmbeddings has the same structure as SigLIP, so the PI function is reused
POSITION_PI_ENABLED      = True  # text PI extension 77 -> 128
POSITION_PI_PRETRAIN_LEN = 77
...
# InfoNCE (CE, row-mean) scale; healthy is about log(global_img)
...
GRADIENT_CHECKPOINTING = True  # with GC off the peak is ~155GB, so it is required to avoid OOM

LOGIT_SCALE_CLAMP_ENABLED = True
LOGIT_SCALE_MAX           = 50.0  # 100 -> 50: exp=81 makes the softmax overconfident and kills the gradient
LOGIT_SCALE_FREEZE        = False

CROSS_GPU_NEG_GATHER = False  # single GPU, so the negatives are the local NxN batch

UNIFORMITY_REG_ENABLED = True
...
def _heldout_module():
    """Import heldout_split_v1 from the same directory (lazy)."""
    global _HELDOUT_MOD
    if _HELDOUT_MOD is None:
        here = os.path.dirname(os.path.abspath(__file__))
        if here not in sys.path:
            sys.path.insert(0, here)
        import importlib
        _HELDOUT_MOD = importlib.import_module(HELDOUT_MODULE)
    return _HELDOUT_MOD

def _ensure_heldout_cache(verbose: bool = True) -> dict:
    """Load the pre-built held-out split and return the summary stats."""
    hs = _heldout_module()
    t0 = time.time()
    stats = hs.ensure_heldout_split(verbose=verbose)
    gates = stats.get("gates", {})
    if verbose:
        print(f"  [heldout] cache ready ({time.time() - t0:.0f}s)  "
              f"heldout={stats.get('n_heldout'):,}  queries={stats.get('n_queries'):,}  "
              f"gallery easy/hard={stats.get('n_gallery_easy'):,}/{stats.get('n_gallery_hard'):,}")
        print(f"  [heldout] candidate pool {stats.get('pct_candidate_of_total')}% of train "
              f"(large near-duplicate components {stats.get('excluded_from_candidacy'):,} images cannot be held out = sample bias)")
        _bm = (f"boundary cos>={gates.get('g2_gate_thr')} pairs {gates.get('g2_cross_neardup_pairs')}"
               if str(gates.get("g2_sim_mode", "")).startswith("embedding")
               else f"boundary min hamming {gates.get('g2_boundary_min_hamming')}")
        print(f"  [heldout] gates: {_bm}  sim={gates.get('g2_sim_mode')}  "
              f"all_pass={gates.get('all_pass')}")
    if HELDOUT_REQUIRE_GATES and not gates.get("all_pass"):
        raise RuntimeError(
            f"heldout gate failed - aborting training. gates={json.dumps(gates, ensure_ascii=False)}")
    return stats
...
def _siglip2_normalize_text(text: Optional[str]) -> str:
    """MetaCLIP2/XLM-V is cased multilingual, so only .strip() is applied, without lowercasing."""
    if text is None:
        return ""
    return text.strip()
...
def _first_batch_lowercase_verify(texts, where: str):
    """Cased text is allowed, so this is a no-op."""
    return
    # the SigLIP2 lowercase check below is unused
...
class SelfResidualBatchSampler(torch.utils.data.Sampler):
    """Weighted seed draw plus self-residual hard-negative neighbour co-location (single GPU)."""
    def __init__(self, weights, neighbors, batch_size, num_batches,
                 co_locate_frac, neighbors_per, generator):
        self.w = weights; self.nb = neighbors
        self.bs = int(batch_size); self.nbatch = int(num_batches)
        self.frac = float(co_locate_frac); self.nper = int(neighbors_per)
        self.gen = generator; self.N = len(neighbors)
    def __len__(self): return self.nbatch
    def __iter__(self):
        for _ in range(self.nbatch):
            if self.w is not None:
                seeds = torch.multinomial(self.w, self.bs, replacement=True, generator=self.gen).tolist()
            else:
                seeds = torch.randint(0, self.N, (self.bs,), generator=self.gen).tolist()
            coins = torch.rand(self.bs, generator=self.gen).tolist()
            batch = []; seen = set()
            for s, coin in zip(seeds, coins):
                if len(batch) >= self.bs: break
                if s not in seen: seen.add(s); batch.append(s)
                if coin < self.frac and self.nb[s] and len(batch) < self.bs:
                    nbrs = self.nb[s]
                    for _i in range(self.nper):
                        if len(batch) >= self.bs: break
                        nidx = nbrs[int(torch.randint(0, len(nbrs), (1,), generator=self.gen).item())]
                        if nidx not in seen: seen.add(nidx); batch.append(nidx)
            while len(batch) < self.bs:
                s = int(torch.randint(0, self.N, (1,), generator=self.gen).item())
                if s not in seen: seen.add(s); batch.append(s)
            yield batch[:self.bs]

def _build_v0031_neighbors_ds(sampler_rows):
    """neighbor file (image keys) -> a neighbour list ordered by dataset index."""
    nb = torch.load(V0031_NEIGHBOR_FILE, map_location="cpu", weights_only=False)
    nb_img2idx = {img: i for i, img in enumerate(nb["images"])}
    ds_imgs = [r["image"] for r in sampler_rows]
    ds_img2idx = {img: i for i, img in enumerate(ds_imgs)}
    out = [[] for _ in range(len(ds_imgs))]
    for di, img in enumerate(ds_imgs):
        ni = nb_img2idx.get(img)
        if ni is None: continue
        for j in nb["neighbors"][ni]:
            t = ds_img2idx.get(nb["images"][j])
            if t is not None: out[di].append(t)
    return out
...
def build_train_loader(train_rows, tokenizer, train_tf):
    ...
    if V0031_ENABLED and os.path.isfile(V0031_NEIGHBOR_FILE):
        # self-residual hard-negative batch co-location; loss and model unchanged
        neighbors_ds = _build_v0031_neighbors_ds(sampler_rows)
        w = sampler.weights.double() if sampler is not None else None
        num_batches = max(1, int(round(len(sampler_rows) * EPOCH_LEN_FACTOR / BATCH_SIZE)))
        bsampler = SelfResidualBatchSampler(w, neighbors_ds, BATCH_SIZE, num_batches,
                                            V0031_CO_LOCATE_FRAC, V0031_NEIGHBORS_PER, sampler_gen)
        avg_nb = sum(len(x) for x in neighbors_ds) / max(1, len(neighbors_ds))
        n_has = sum(1 for x in neighbors_ds if x)
        print(f"  [sampler] SelfResidualBatchSampler: co_locate={V0031_CO_LOCATE_FRAC} per={V0031_NEIGHBORS_PER} "
              f"| with neighbours {n_has:,}/{len(neighbors_ds):,} avg={avg_nb:.2f} | num_batches={num_batches:,}", flush=True)
        loader = DataLoader(
            train_ds, batch_sampler=bsampler,
...
def _unfreeze_logit_params(peft_model):
    """Make only logit_scale trainable (its own LR group). CLIP has no logit_bias."""
    for name, p in peft_model.named_parameters():
...
def build_model():
    """Build the MetaCLIP2 + DoRA + FLAIR model."""
    from transformers import AutoModel, AutoTokenizer
    from peft import LoraConfig, get_peft_model

    # tf5.4 cannot resolve the repo id offline, so use the local snapshot path
    import glob as _glob
    def _resolve_local(repo):
        base = f"{os.environ.get('HF_HOME', HF_CACHE_DIR)}/hub/models--{repo.replace('/','--')}/snapshots"
        for s in sorted(_glob.glob(base + "/*")):
            if os.path.isfile(os.path.join(s, "config.json")):
                return s
        return repo
    _local = _resolve_local(MODEL_NAME)
    print(f"[model] loading {MODEL_NAME} ← {_local} ...")
    base_model = AutoModel.from_pretrained(_local, low_cpu_mem_usage=True)
    tokenizer  = AutoTokenizer.from_pretrained(_local)
    ...
    if POSITION_PI_ENABLED:
        print("\n[step 1] Text Position Interpolation 77→128 ...")
    ...
    # DoRA wrap - vision+text union, text rank override (r=16/alpha=32)
    ...
    # HF MetaClip2 text is CLIP-style '.layers' (12), q/k/v/out_proj and fc1/fc2
...
def clip_infonce_loss(v, t, image_ids, logit_scale):
    """symmetric InfoNCE (single-GPU reference path)."""
    v = F.normalize(v.float(), dim=-1)
    t = F.normalize(t.float(), dim=-1)
...
def compute_step_loss(v, t, image_ids,
    ...
    """Call symmetric InfoNCE. Single GPU, so always the local NxN reference path."""
...
class JointEncoder(nn.Module):
    ...
    def _encode(self, pixel_values, input_ids, attention_mask):
        """One vision/text forward plus a manual projection."""
        ...
        v = self.base.visual_projection(vo.pooler_output)
        t = self.base.text_projection(to.pooler_output)
        return v, t, vo, to
    ...
    def forward(self, pixel_values, input_ids, attention_mask):
        if self.flair_enabled:
            ...
            # pooler tok_dim = text hidden_size, so the query is unprojected text too
...
def train_single(physical_gpu: int,
    ...
    try:
        ...
        # excluding per image removes all of its captions, so text leakage is zero
        # self-residual neighbours drop out of the dataset index mapping automatically
        heldout_summary = None
        if USE_HELDOUT_SPLIT:
            print("\n[1b/5] Heldout split (DINOv2 components of the source data) ...")
            heldout_summary = _ensure_heldout_cache()
            heldout_imgs = _heldout_module().load_heldout_images(build_if_missing=False)
            before = len(train_rows)
            train_rows = [r for r in train_rows if r["image"] not in heldout_imgs]
            n_excluded = before - len(train_rows)
            print(f"  heldout images {len(heldout_imgs):,} -> excluded from the manifest {n_excluded:,} "
                  f"(train {before:,} → {len(train_rows):,}, "
                  f"-{100.0 * n_excluded / max(1, before):.2f}%)")
            if n_excluded != len(heldout_imgs):
                raise RuntimeError(
                    f"heldout exclusion count mismatch: excluded={n_excluded} != heldout={len(heldout_imgs)}. "
                    f"Check whether the manifest image keys use the same format as the split.")
            assert not (set(r["image"] for r in train_rows) & heldout_imgs), "heldout leak in train_rows"
            metric_writer.log(event="heldout_applied", n_heldout=len(heldout_imgs),
                              n_excluded=n_excluded, n_train=len(train_rows),
                              heldout_md5=heldout_summary.get("heldout_md5"),
                              gates_all_pass=heldout_summary.get("gates", {}).get("all_pass"))
        ...
        # attach happens *after* the freeze in build_model, so it stays trainable
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            for step, batch in enumerate(loop, 1):
                ...
                with torch.amp.autocast("cuda", dtype=AMP_DTYPE, enabled=USE_AMP):
                    ...
                    if step == 1:
                        print(f"  [batch-sanity] local_txt={t.shape[0]} local_img={v.shape[0]} "
                              f"(InfoNCE CE row-mean, single-GPU NxN)")

                    # symmetric InfoNCE (single GPU, so the gather is a no-op)
                ...
                if _FG_OR_FLAIR and step == 1:
                    # in CLIP, pooler_output (pre-proj) != get_*_features (post-proj)
                    # checks that the manual projection agrees
                    ...
                    try:
                        ...
                        print(f"    [verify] manual-proj vs get_*_features (eval) max|diff| "
...
def check_inputs() -> None:
    ...
    if USE_HELDOUT_SPLIT:
        try:
            _ensure_heldout_cache(verbose=False)
        except Exception as e:
            problems.append(f"heldout split unusable: {e}")
    if V0031_ENABLED and not os.path.isfile(V0031_NEIGHBOR_FILE):
        problems.append(f"self-residual neighbour file not found: {V0031_NEIGHBOR_FILE}")
...
def main():
    parser = argparse.ArgumentParser(
        description="MetaCLIP2-H/14@378 + DoRA + FLAIR - held-out excluded (single GPU)")
