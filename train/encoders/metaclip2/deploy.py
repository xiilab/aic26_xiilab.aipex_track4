#!/usr/bin/env python3
"""Copy the SWA checkpoint of a metaclip2 run to the reproduction deployment location.

  build_swa.py : <run> -> checkpoints/swa
  this script  : <run> -> assets/model_rep/encoder/metaclip2

The name is this directory's name. Only the files inference needs are copied
(README.md and swa_meta.json are excluded). An existing deployment is removed and rewritten.

usage (run from the repository root):
  python train/encoders/metaclip2/deploy.py <run>
"""
...
def main():
    ...
    ap.add_argument("run", help="run directory (must hold checkpoints/swa)")
    args = ap.parse_args()
    ...
    if not os.path.isdir(run):
        ...
        problems.append(f"SWA not found: {os.path.relpath(src, _REPO)}\n"
                        f"      build: python train/encoders/{NAME}/build_swa.py {args.run}")
    else:
