"""MetaCLIP2-L/14@224 + DoRA(v8/t16) + PI(77->128) + FLAIR, full split, DDP.

Reproduces the deployed metaclip2 encoder: 5-GPU DDP, batch 128 per rank (global 640),
4 epochs on a 15-epoch cosine horizon. There is no early stop - EPOCHS is the only exit.

data      assets/data/manifest/pab_manifest_msr_v1.jsonl (built by train/gen/gen_manifest.py;
          the same captions anchor_filip uses). Read only; the challenge test set is never opened.
adapter   DoRA r=8/alpha=16 on the vision encoder, r=16/alpha=32 on the text encoder,
          targets q/k/v/out_proj and fc1/fc2. The backbone stays frozen.
text      MetaCLIP2 text is CLIP-style with 77 pretrained position rows; Position Interpolation
          maps 128 input positions onto them, and all 77 rows stay frozen.
pooling   FLAIR text-conditioned attention pooling, train only. Inference falls back to global v.
LR        every group is scaled by sqrt(world_size), so 7e-5 becomes 1.57e-4 at world_size=5.

usage (run from the repository root):
  python train/encoders/metaclip2/train.py --gpus 0,1,2,3,4 --run-note metaclip2
  python train/encoders/metaclip2/train.py --gpus 0,1,2,3,4 --resume <ckpt>

next steps:
  python train/encoders/metaclip2/build_swa.py <run>    # averages ep02-ep04
  python train/encoders/metaclip2/deploy.py    <run>    # -> assets/model_rep/encoder/metaclip2
...
from torchvision.transforms import v2 as Tv2          # tensor-domain transform (faster training augmentation)
from PIL import Image
from tqdm import tqdm

import cv2                                            # faster JPEG decode (~1.5x over PIL)
...
# =========================================================
# 1. Config - every parameter is a module constant (no argparse)
# =========================================================
...
MANIFEST_TAG        = "v1"                        # msr caption generation; produced by gen_manifest.py
MANIFEST_PATH       = f"{MANIFEST_DIR}/pab_manifest_msr_{MANIFEST_TAG}.jsonl"

# unique image mode: __getitem__ picks one caption at random per image (a new view each epoch)
USE_UNIQUE_IMAGE_MODE = True
RUNS_ROOT       = "assets/runs"
RUN_NOTE        = os.path.basename(os.path.dirname(os.path.abspath(__file__)))  # {timestamp}_{directory name} when --run-note is omitted


# ---- model ----
MODEL_NAME      = "facebook/metaclip-2-worldwide-l14"   # MetaCLIP2 ViT-L/14 worldwide (vision hidden 1024, text XLM-V-base, 224px)
IMAGE_SIZE      = 224                                       # MetaCLIP2 ViT-L/14 (patch14 -> 16x16 grid)
MAX_TEXT_LENGTH = 128                                       # MetaClip2 text is pretrained at 77 (CLIP-style); PI extends it to 128
                                                            #   14% of PAB queries exceed 77 tokens; without PI they would be truncated
POSITION_EMBED_NEW_SIZE = 128                               # *logical* target length (= PI target); PI adds no rows

# ---- DoRA ----
DORA_RANK       = 8                                         # vision rank
DORA_ALPHA      = 16                                        # alpha = 2 * rank
DORA_RANK_TEXT  = 16                                        # text override (rank_pattern)
DORA_ALPHA_TEXT = 32
DORA_DROPOUT    = 0.10
DORA_TARGETS    = ["q_proj", "k_proj", "v_proj", "out_proj", "fc1", "fc2"]
# separate vision/text DoRA target module names; the config uses their union
DORA_TARGETS_VISION = ["q_proj", "k_proj", "v_proj", "out_proj", "fc1", "fc2"]
DORA_TARGETS_TEXT   = ["q_proj", "k_proj", "v_proj", "out_proj", "fc1", "fc2"]  # MetaClip2 text is CLIP-style too (same as vision); the union is this list
FREEZE_VISION_DORA = False                                  # False trains the vision encoder DoRA
FREEZE_POOLER_DORA = False                                  # False trains the pooler (SplitMHA) DoRA

# ---- Multi-Probe Pooling Head (split design) ----
# One probe (1,1,D) -> K probes + aggregator. probe[0] is the preserved clone,
# probe[1..K-1] are relative perturbations; the aggregator uses identity_warmup.
# Inert at NUM_PROBES=1: the surgery is skipped and the single-probe head stays.
NUM_PROBES               = 1                # K=1 (multi-probe off, single-probe head)
...
MULTI_PROBE_BLOCK_SCALE  = 0.1              # starting scale of block_1..K-1
TRAIN_MULTI_PROBE_PARAMS = True
LR_MULTI_PROBE           = 7e-5             # sqrt scaling: 1e-4 * sqrt(128/256) ~= 7e-5
WD_MULTI_PROBE           = 0.0              # identity_warmup protection (standard LoRA practice)

# attention-based aggregator (per-sample probe selection)
MULTI_PROBE_AGG_TYPE     = "attention"      # "linear" | "attention" (default)
MULTI_PROBE_ATTN_TEMP    = 1.0              # softmax temperature (lower is sharper)

# separate LR for probe and aggregator
LR_MULTI_PROBE_PROBE_SPLIT = True           # True gives probe and aggregator separate LRs
LR_MULTI_PROBE_PROBE       = 3.5e-5         # sqrt scaling: 5e-5 * sqrt(128/256) ~= 3.5e-5
LR_MULTI_PROBE_AGG         = 7e-5           # sqrt scaling: 1e-4 * sqrt(128/256) ~= 7e-5

# probe diversity regularization (off-diagonal cosine penalty); inert at K=1
ENABLE_PROBE_DIVERSITY_REG    = False       # off at K=1, stated explicitly
PROBE_DIVERSITY_REG_LAMBDA    = 2e-3        # dead (K=1)
PROBE_DIVERSITY_WARMUP_EPOCHS = 2           # dead (K=1)

# multi-positive K (captions per image)
# memory: batch 128 with K=2 gives 256 texts; without GC the peak is ~155GB and OOMs
N_POSITIVES_PER_IMAGE    = 2

# ---- training ----
SEED            = 42
EPOCHS          = 4     # hard stop; the epoch loop has no other exit
# the cosine T_max follows SCHEDULE_HORIZON_EPOCHS, not EPOCHS: training continues to EPOCHS
# but stays flat at eta_min past the horizon. Lowering EPOCHS does not move the LR curve.
SCHEDULE_HORIZON_EPOCHS = 15
BATCH_SIZE      = 128   # the LR base (7e-5) assumes batch 128 per GPU, matching the sqrt(ws) scaling
                        #   negatives also grow with the global batch (128 x 5 = 640)
NUM_WORKERS     = 4
EVAL_NUM_WORKERS = 4
EVAL_BATCH_SIZE = 256   # no-grad inference; kept conservative
LR              = 7e-5                                      # sqrt scaling: 1e-4 * sqrt(128/256) ~= 7e-5
LR_LOGIT        = 1e-5

# ==== Fine-grained token<->patch matching (FILIP-style) ====
#   A light projection on the token/patch hidden states in the same forward gives a
#   token-wise max-sim late-interaction loss, added to the global loss with lambda.
FINEGRAIN_ENABLED     = False      # mutually exclusive with FLAIR, which replaces it
FINEGRAIN_WEIGHT      = 0.5        # lambda: L = L_global + lambda*L_filip
FINEGRAIN_DIM         = 256        # late-interaction projection dim (small; only the head is newly trained)
...
FINEGRAIN_TOK_MAXTHR  = 0          # >0 guards against OOM when batch*Lt grows (0 = off)
FINEGRAIN_AGG_RATIO   = 0.06       # k as a fraction of Lp (Lp=64 -> k=4); adapts to a resolution or pool change
FINEGRAIN_TARGET_LP   = 64         # FILIP late-interaction resolution (8x8); changing Lp is a different experiment
# startup fail-fast: check that IMAGE_SIZE and the pool give the intended Lp
# (guarded so a new IMAGE_SIZE cannot fail while FINEGRAIN_ENABLED=False)
...
LR_FINEGRAIN          = 2e-4       # projection head LR (new trainable, separate from the base)
# jigsaw (pixel shuffle + permutation prediction): off, it doubles the vision forward
JIGSAW_ENABLED        = False

# ==== FLAIR text-conditioned attention pooling ====
#   Cross-attention with the pooled text token as query and vision patches as K/V.
#   Train only: inference drops the pooler and falls back to global v.
FLAIR_ENABLED      = True      # mutually exclusive with FINEGRAIN_ENABLED (asserted below)
FLAIR_WEIGHT       = 0.30      # lambda_flair in L = L_global + lambda_flair * L_tcs (target after warmup)
FLAIR_WEIGHT_WARMUP_EPOCHS = 3 # epochs of linear lambda warmup (0 = none)
FLAIR_WEIGHT_WARMUP_START  = 0.0  # starting lambda for the warmup, rising linearly to FLAIR_WEIGHT
FLAIR_DIM          = 256       # cross-attention inner dim (only the head is newly trained)
FLAIR_NUM_HEADS    = 4         # multi-head (FLAIR_DIM % FLAIR_NUM_HEADS == 0)
FLAIR_INPUT_LN     = True      # LayerNorm on the pooler inputs; damps attention peakiness at init
FLAIR_LOGIT_SCALE  = 20.0      # fixed temperature (not a parameter, so no DDP logit sync)
FLAIR_PATCH_POOL   = 2         # 2D avgpool downsample of v_loc (grid/pool -> Lp) to bound memory
FLAIR_TARGET_LP    = 64        # intended Lp after pooling (8x8); used by the fail-fast check
FLAIR_STRIP_CLS    = True      # drop CLS (index 0) from the CLIP last_hidden_state, keeping patches only
FLAIR_GRID_DIV     = 14        # patch14: grid = IMAGE_SIZE // 14 (224//14 = 16)
FLAIR_SYMMETRIC    = True      # symmetric text->image + image->text CE (handles K>1 multi-positive)
FLAIR_LOSS_ABORT_THRESH = 15.0 # loss_tcs divergence abort threshold (healthy CE ~= log(Ni) ~= 4.8)
LR_FLAIR           = 2e-4      # pooler LR (new trainable, separate from the base)
...
if FLAIR_ENABLED:
    _flair_grid = IMAGE_SIZE // FLAIR_GRID_DIV          # patch14 grid (224//14 = 16)
...
LR_POSITION     = 5e-5                                      # no effect when POSITION_PRETRAINED_FREEZE=True (the grad hook zeroes it)
                                                            # ---- DDP LR auto-scaling ----
                                                            # The LR constants above assume world_size=1 at BATCH_SIZE=128. build_optimizer multiplies
                                                            # every group LR by lr_scale. The EMA half-life is epoch-based and needs no correction.
DDP_LR_REF_WORLD_SIZE = 1                                   # the single-GPU path
DDP_LR_SCALING        = "sqrt"                              # "sqrt" | "linear" | "none"
# the row count stays at the pretrained length - there are no extended rows
# with POSITION_PRETRAINED_FREEZE=True all of them are frozen and DoRA adapts instead
POSITION_PRETRAINED_FREEZE = True                           # the 77 pretrained rows stay frozen; PI interpolates and DoRA adapts
POSITION_PRETRAINED_SIZE   = 77                              # MetaClip2 text max_position_embeddings (CLIP-style learned pos emb)
# ---- Position Interpolation (PI) ----
# MetaClip2 text uses CLIP-style learned position embeddings with 77 rows, and 14% of PAB
# queries exceed 77 tokens, so PI extends 77 -> 128. The embedding layout matches SigLIP,
# so the SigLIP PI helpers are reused: map [0..L-1] onto [0..76] and blend the two rows.
POSITION_PI_ENABLED      = True                              # text PI extension 77 -> 128
POSITION_PI_PRETRAIN_LEN = 77                                # MetaClip2 text pretrained max_position_embeddings
POSITION_PI_TARGET_LEN   = 128                               # = MAX_TEXT_LENGTH, the usable length after the extension
LR_MIN_RATIO    = 0.02
WEIGHT_DECAY    = 0.01
GRAD_CLIP       = 1.0
# ---- gradient-spike rejection: drop destructive steps and dump forensics ----
#   grad_norm is 0.3-1.6 when healthy and 12-17 just before collapse (ep1 warmup excluded)
GRAD_SPIKE_SKIP   = True
GRAD_SPIKE_THRESH = 5.0
USE_AMP         = True
AMP_DTYPE       = torch.bfloat16
DROP_LAST       = True
WARMUP_RATIO    = 0.05                                      # fraction of the horizon (adapts automatically when BATCH changes)
...
# ---- safety guards ----
# InfoNCE (CE, row-mean) scale; healthy is about log(global_img), so 20.0 means divergence
LOSS_EXPLOSION_THRESHOLD = 20.0
LOSS_NAN_ABORT           = True

# ---- image normalization (OpenAI CLIP) ----
IMG_NORM_MEAN = (0.48145466, 0.4578275, 0.40821073)
IMG_NORM_STD  = (0.26862954, 0.26130258, 0.27577711)

GRADIENT_CHECKPOINTING = True                               # without GC the peak is ~155GB and OOMs

# =========================================================
# Divergence guards
# =========================================================
# ---- logit_scale clamp ----
# An unclamped logit_scale saturates (exp ~ 100) and the gradient vanishes. Clamping after
# optimizer.step() keeps it trainable but bounded; LOGIT_SCALE_FREEZE is the alternative.
LOGIT_SCALE_CLAMP_ENABLED = True                            # clamp the exp upper bound right after optimizer.step()
LOGIT_SCALE_MAX           = 100.0                           # upper bound on exp(logit_scale) via clamp_(max=log(100)), the CLIP standard
LOGIT_SCALE_FREEZE        = False                           # True freezes logit_scale; pointless together with the clamp

# ---- Cross-GPU negative all-gather ----
# Rank-local negatives alone cost a factor of world_size. v/t/image_ids are all-gathered
# right before the loss; only the local rank keeps grad and image_ids get a rank offset.
# The loss stays NxN with N scaled by world_size. A no-op at world_size==1.
CROSS_GPU_NEG_GATHER = True

# ---- Uniformity regularization (Wang & Isola) - off by default ----
# Computed on rank-local v,t (not the gathered tensors) to save memory.
UNIFORMITY_REG_ENABLED = True                               # uniformity on
UNIFORMITY_REG_LAMBDA  = 0.01                               # lambda_uni ~= 0.01
...
EMA_START_EPOCH      = 2                                    # ep1 is paused (init residue); reset the shadow when ep2 starts
# EMA: the shadow is refreshed every epoch and the ckpt stores it
EMA_DIAGNOSTIC_RAW_EVAL = False   # unused

# ---- Soft label (hierarchical action x scene, false-negative mitigation) ----
# Extends the hard {-1, +1} of SigLIP to a graded p in [0, 1]:
#   loss = -[p * logsigmoid(x) + (1-p) * logsigmoid(-x)]
#   same image_id        -> p = 1.0
#   same (action, scene) -> p = SAME_AS
#   same action only     -> p = SAME_ACTION
#   otherwise            -> p = 0.0
# SAME_SCENE=0.0 is deliberately dead: a scene-only match counts as a negative.
# Disabled here (USE_SOFT_LABEL=False), which makes the loss vanilla multi-positive.
USE_SOFT_LABEL              = False                         # soft label off (vanilla multi-positive)
SOFT_LABEL_SAME_AS          = 0.55                          # dead (USE_SOFT_LABEL=False)
SOFT_LABEL_SAME_ACTION      = 0.25                          # dead
SOFT_LABEL_SAME_SCENE       = 0.0                           # dead
SOFT_LABEL_MISSING_FALLBACK = 0.0                           # dead
SOFT_LABEL_DIAG_ENABLED     = False                         # soft label off, so the diagnostic is off too

# ---- Caption augmentation ----
# The recap variants are the main channel (10 styles x a different view per epoch).
# Sentence-order shuffling adds robustness for multi-sentence queries.
CAPTION_AUGMENT_TRAIN = True
CAPTION_AUGMENT_PROBS = {
    "original":    0.85,
    "order":       0.15,
}
assert abs(sum(CAPTION_AUGMENT_PROBS.values()) - 1.0) < 1e-6, "augment probs must sum to 1.0"

# ---- Image augmentation (protects caption alignment, single noise channel) ----
# HFlip and Grayscale are off: they would contradict direction and colour words.
# RRC and Erase stay mild to preserve the entity.
# Gaussian noise is the only sim2real sensor channel (blur/JPEG cost more for no gain).
IMG_AUG_COLOR_JITTER  = (0.4, 0.4, 0.4, 0.1)
IMG_AUG_HFLIP_PROB    = 0.0                                 # blocks the left/right mismatch present in 12% of captions
IMG_AUG_GRAYSCALE_P   = 0.0                                 # blocks a colour-word mismatch, present in 95% of captions
IMG_AUG_GNOISE_P      = 0.20
IMG_AUG_GNOISE_STD    = (0.005, 0.025)
IMG_AUG_RRC_ENABLED   = False                               # off: deterministic Resize+CenterCrop, same as inference
...
IMG_AUG_ERASE_SCALE   = (0.02, 0.05)                        # small regions only

# ---- data sampling / balance ----
# train anomaly:normal is 1.69:1, so the batch is forced to 1:1
BALANCE_LABEL_TYPE  = True
# anomaly action long-tail (Falling 36%) -> inverse-freq^alpha smoothing.
BALANCE_ACTION      = True
...
# ---- monitoring ----
LOG_EVERY_STEPS  = 50
# raised from 50 to cut the cost of recomputing v@t.T in sim_diagnostics
DIAG_EVERY_STEPS = 200
EVAL_EVERY_EP    = 1
# every epoch is saved and only the last KEEP_LAST_N_CKPT are kept, plus best/last
# N=0 disables per-epoch checkpoints entirely
...
def set_seed(seed: int):
    ...
    cuDNN heuristics are off, which costs roughly 5-10% throughput.
    """
    random.seed(seed)
    np.random.seed(seed)
...
def map_annotation_path_to_local(ann_path: str, root: str = IMG_ROOT) -> str:
    ...
    n = int(parts[1].replace("imgs_", ""))                   # 8
    part_no = n // 8 + 1                                     # 2
    fname = parts[3]                                         # keep the ".jpg" suffix
    return f"{root}/Part {part_no}/{parts[1]}/{parts[2]}/{fname}"

def stable_hash(s: str) -> int:
    """string -> deterministic integer, for combining with a seed."""
    return int(hashlib.sha1(s.encode("utf-8")).hexdigest()[:12], 16)

# scene aliases, merged only where the meaning is clearly identical
_SCENE_ALIAS = {
    "indoor setting":     "indoor",
    "indoor gym":         "indoor gym",         # kept: distinct from gym
    "indoor flea market": "indoor flea market", # kept: specific
...
def normalize_scene(s: str) -> str:
    ...
    An empty string or None maps to "" and is treated as unknown.
    """
    if not s:
        return ""
    s = s.strip().lower()
...
# =========================================================
# 2.1 Logging utilities - structured logs for later analysis
# =========================================================
...
def collect_env_info():
    ...
    for pkg in ["transformers", "peft", "torchvision", "PIL", "numpy"]:
        try:
            ...
            info[pkg] = None
    # git hash (optional; a failure here is harmless)
    try:
        import subprocess
...
def snapshot_config():
    """Snapshot the module-level UPPER_CASE config variables into a dict, skipping
    module/class/function objects.
    """
    import types
    g = globals()
    cfg = {}
...
def diagnose_multi_probe(model):
    ...
    Returns an empty dict at K<=1 or when there is no multi-probe head.
    """
    ...
    # eps=1e-6 keeps a collapsed (near-zero) norm from producing NaN; diagnostics only
    ...
    # branch on the aggregator type
    agg_type = getattr(head, "agg_type", "linear")
    if agg_type == "linear":
        # aggregator block norm (the K blocks of a single Linear)
        ...
        # attention aggregator diagnostics: agg_query norm, agg_init_bias spread, agg_proj deviation from I
...
def compute_sim_diagnostics(v, t, image_ids=None):
    """SigLIP positive vs negative similarity statistics. With image_ids the positives are
    multi-positive; without them the diagonal of sim is used.
    """
    ...
    return {
        ...
        "v_norm_mean":   v.norm(dim=-1).mean().item(),       # ~1.0 (normalized)
...
def load_manifest():
    ...
    The manifest rows are ordered by annotation file name (imgs_0, imgs_1, imgs_10 ...), while
    this trainer has always enumerated them numerically (imgs_0, imgs_1, imgs_2 ...), so the
    order is restored with a stable sort after reading. Row order changes which indices the
    WeightedRandomSampler draws. The file itself is untouched, so trainers that share the same
    ...
    with open(MANIFEST_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            rows.append(r)
    rows.sort(key=lambda r: int(r["image"].split("/")[1][5:]))   # imgs_N numeric order (stable sort)
...
class GaussianNoiseAug:
    """Gaussian noise in the tensor domain (sim2real sensor noise), applied on the
    [-1, 1] range after Normalize.
    """
    ...
    def __call__(self, tensor):
        # tensor: (C, H, W), [-1, 1] after Normalize
        if random.random() >= self.p:
            return tensor
        std = random.uniform(self.std_range[0], self.std_range[1])
        noise = torch.randn_like(tensor) * std
        # the CLIP norm range is not [-1,1], so the clamp is removed
        return tensor + noise
...
def _cv2_load_image(path, target_size: Optional[int] = None) -> torch.Tensor:
    ...
    Raises IOError on failure; PABDataset.__getitem__ handles the fallback.
    """
    ...
    if target_size is not None and (img.shape[0] != target_size or img.shape[1] != target_size):
        img = cv2.resize(img, (target_size, target_size),
                         interpolation=cv2.INTER_AREA)   # best for downscaling (400 -> 384/224 and similar)
...
def build_train_transform(image_size=IMAGE_SIZE):
    ...
    Input is a uint8 (3, H, W) tensor from _cv2_load_image().
    """
    transforms = []
    ...
    # otherwise _cv2_load_image already resized to image_size
    if IMG_AUG_HFLIP_PROB > 0:
        transforms.append(Tv2.RandomHorizontalFlip(IMG_AUG_HFLIP_PROB))
    transforms.append(Tv2.ColorJitter(*IMG_AUG_COLOR_JITTER))   # uint8 tensor vector op
    ...
    transforms.append(Tv2.Normalize(mean=IMG_NORM_MEAN, std=IMG_NORM_STD))  # OpenAI CLIP norm
...
class PABDataset(Dataset):
    ...
    With is_training=True the caption augmentation applies. In unique_image_mode every
    __getitem__ draws captions at random from the image's caption group, so each epoch is a
    different view and one epoch covers the unique images once.
    """
    ...
    def __init__(self, rows, transform, is_training=False,
        ...
        # records are already per image (captions list), so no grouping is needed
        ...
        # image read failure counter - tracks silent placeholder injection
        # persistent_workers means each worker accumulates, so the print is rate-limited
    ...
    def __getitem__(self, idx):
        # pick K positive captions per image, from the caption pool
        rec = self.rows[idx]
        r_rep = rec
        K = self.n_positives
        if isinstance(rec.get("captions"), list):
            pool = rec["captions"]                                  # image-level dieted
        elif self.unique_image_mode:
            pool = [r["caption"] for r in self.image_groups[rec["image"]]]  # row-level
        else:
            pool = [rec["caption"]]
        if len(pool) >= K:
        ...
        # cv2 decode + resize -> uint8 (3, H, W)
        # with RRC on, load at native resolution and let RandomResizedCrop handle it
        ...
        # returns K captions (list[str]); at K=1 this equals the "text" field below
        texts: list = []
        for c in sel_caps:
            if self.is_training and CAPTION_AUGMENT_TRAIN:
                texts.append(augment_caption_train(c))
            else:
                texts.append(c)
...
# --- SigLIP2 lowercase guards (unused on MetaCLIP2, kept for reference) ---
#   _siglip2_normalize_text / _assert_lowercase / _LowercaseTokenizerWrapper /
#   _first_batch_lowercase_verify
def _siglip2_normalize_text(text: Optional[str]) -> str:
    """MetaCLIP2/XLM-V is cased multilingual, so only .strip() is applied."""
    if text is None:
        return ""
    return text.strip()

def _assert_lowercase(texts, where: str = "?"):
    """Cased text is allowed, so this is a no-op."""
    return

class _LowercaseTokenizerWrapper:
    """SigLIP2 lowercase wrapper: normalize automatically before every __call__.
    A wrapper class with __getattr__ delegation, because __call__ is looked up on the class.
    """
...
class _TrainCollator:
    """Module-level picklable collate callable. A local closure cannot be pickled when the
    DataLoader workers use the spawn context, so this is a class.
    """
    ...
    def __call__(self, batch):
        ...
        # text side repeats image_ids K times
...
def make_collate(tokenizer, max_length=MAX_TEXT_LENGTH, n_positives=1):
    ...
    The K captions of each image are flattened, so the text batch is N x K and
    image_ids is [0,0,...,1,1,...,N-1,N-1,...].
    """
...
def compute_sample_weights(rows,
    ...
    Level A equalises the total weight per label_type, giving a 1:1 normal:anomaly batch in
    expectation; level B smooths the action distribution inside each label_type.
    """
    label_count  = Counter(r["label_type"]   for r in rows)
    action_count = Counter(r["action_label"] for r in rows)

    # Level B: action smoothing
    weights = np.ones(len(rows), dtype=np.float64)
    if balance_action and action_alpha > 0:
        for i, r in enumerate(rows):
            weights[i] = 1.0 / max(1.0, action_count[r["action_label"]]) ** action_alpha

    # level A: normalize per label_type so each has the same expected sampling mass
    if balance_label:
...
def worker_init_fn(worker_id):
    ...
    With persistent_workers=True the init runs once per worker at spawn.
    """
...
def _verify_unique_image_invariants(train_ds):
    """Verify the sampler invariants of USE_UNIQUE_IMAGE_MODE: the image-level weight only
    sees the representative row, so every row of an image must share label_type and action.
    """
    if getattr(train_ds, "image_groups", None) is None:
        # image-level records carry one meta per image, so label/action are invariant by construction
        n_unique = len(train_ds.unique_images)
        n_cap = sum(len(r["captions"]) for r in train_ds.rows)
        print(f"  [v23-diet] unique_image_mode: {n_unique:,} images, {n_cap:,} captions "
...
def build_train_loader(train_rows, tokenizer, train_tf):
    """The training DataLoader. With USE_UNIQUE_IMAGE_MODE=True the sampler weights are per image
    (representative row). The sampler generator is exposed so the epoch loop can reseed it.
    """
    ...
    # the epoch reseed in the main loop reaches loader.sampler.generator, or loader.generator
    # when there is no sampler - the standard API rather than a private attribute
    return loader, label_count, action_count

# =========================================================
# 6. Model - MetaCLIP2 + Position Interpolation (PI) + DoRA
# =========================================================
# Position Interpolation instead of growing the learned absolute position embedding:
#   the row count stays at the pretrained length, input positions [0..L-1] are scaled to
#   p' = p * (P-1)/(T-1) (endpoint-aligned, Chen et al. 2023) and the two neighbouring rows
#   are blended. No new rows, so no sparsity in the extended range.

def _compute_pi_scale(pretrain_len: int, target_len: int) -> float:
    """Endpoint-aligned PI scale: p in [0, T-1] -> p' = p * scale in [0, P-1], scale = (P-1)/(T-1)."""
    if target_len <= 1:
        return 0.0
    return (pretrain_len - 1) / (target_len - 1)

# How the patch works: the module is not replaced, only its forward. token_embedding,
#   position_embedding and the position_ids buffer stay intact, so PEFT, save_pretrained
#   and state_dict loading keep working. The swap is a bound method.
...
def _pi_text_embeddings_forward(self, input_ids=None, position_ids=None, inputs_embeds=None):
    ...
    self is the original embeddings instance and carries _pi_scale / _pi_pretrain_len.
    """
    ...
    if position_ids is not None:
        # caller-supplied position_ids; float and long are both accepted, PI applies before lookup
        pos = position_ids.to(device=device, dtype=torch.float32)
        if pos.dim() == 2:
            pos = pos[0]                                        # (L,); assumed identical across the batch (Siglip2 default)
    else:
        # standard path: fractional scaling
    ...
    # align dtypes: under autocast pe_weight is bf16 while w_lo/w_hi are fp32
    w_lo_c = w_lo.to(e_lo.dtype)
    w_hi_c = w_hi.to(e_hi.dtype)
    position_embeddings = w_lo_c * e_lo + w_hi_c * e_hi          # (L, D)

    # (1, L, D) broadcast on batch via add. inputs_embeds: (B, L, D).
...
def _install_position_freeze_hook(text, n_freeze: int, dim: int, total_rows: int):
    ...
    A sanity snapshot and n_freeze are kept as buffers for the later verification.
    """
...
def apply_position_interpolation(base_model,
    ...
    """Apply Position Interpolation to the text encoder: keep the row count at pretrain_len,
    swap the embeddings forward for the PI version, and attach the scale as an attribute.
    """
    ...
    # idempotency: skip if the patch is already applied
    if getattr(emb, "_pi_applied", False):
        print(f"  [pos-pi] skip: PI already applied (safe re-entry)")
        return base_model

    # replace the forward with a bound method
...
# compatibility alias; callers still use the extend_position_embedding name
def extend_position_embedding(base_model, new_size: int = POSITION_EMBED_NEW_SIZE):
    """Compat shim; handled by Position Interpolation. new_size is read as the PI target_len."""
...
# --- Pooler DoRA surgery ---
# The pooler's nn.MultiheadAttention holds a combined in_proj_weight that PEFT cannot
# wrap, so it is replaced by numerically equivalent split Q/K/V/O Linear layers named
# q_proj/k_proj/v_proj/out_proj so DORA_TARGETS matches them.
# Inference must apply the same surgery before PeftModel.from_pretrained();
# pooler_mha_split=True in meta.json signals that.
class SplitMHAModule(nn.Module):
    """A split-projection version compatible with the nn.MultiheadAttention forward signature
    the vision pooler calls: attn_output, _ = self.attention(probe, hidden, hidden, ...).
    """
    def __init__(self, embed_dim, num_heads, bias=True, dropout=0.0,
        ...
        # named to match DORA_TARGETS so PEFT attaches automatically
    ...
    def forward(self, query, key, value,
        ...
        # returns (out, attn_weights) to match the nn.MultiheadAttention signature; weights unused
        return out, None

def _replace_pooler_mha_with_split(base_model):
    """Replace every nn.MultiheadAttention in vision_model with a SplitMHAModule:
    in_proj_weight (3E, E) splits into q/k/v (E, E) and out_proj is copied as is.
    Numerical equivalence is checked in fp32 and bf16.
    """
    targets = []
    ...
    for parent_name, parent, child_name, old_mha in targets:
        ...
        # align device and dtype so a different build order stays safe
        ...
        # numerical equivalence check (eval mode, isolated RNG)
        ...
        # measure the bf16 autocast numerical gap (informational)
        # called before model.to(DEVICE), so move to GPU temporarily only on CUDA
        if torch.cuda.is_available() and old_dev.type == "cuda":
            ...
            try:
                ...
                # undo a partial move (e.g. old_mha on GPU, new_mha on CPU)
                try:
...
# --- Multi-Probe Pooling Head ---
# One probe (1,1,D) -> K probes + aggregator: each probe cross-attends to a different
# region/aspect and the aggregator merges them back into one vector.
# With aggregator_init=identity and perturb_std=0 this is numerically the single-probe head.
class MultiProbePoolingHead(nn.Module):
    """K-probe + aggregator wrapper (unified design).
    probe (1, K, D): probe[0] is the preserved clone, probe[1..K-1] add a relative perturbation.
    aggregator Linear(K*D, D): block_0 = I/norm and block_k = block_scale*I/norm at init.
    """
    def __init__(self, orig_head, num_probes=4, perturb_std=0.01,
        ...
        # -- probe (unified, K of them) --
        # probe[0] = preserved clone of the original
        # probe[1..K-1] = original + relative perturbation (specialization seed)
        ...
        if num_probes > 1:
            ...
            with torch.no_grad():
                # perturb probe[1:] only; probe[0] stays the original clone
        ...
        # -- existing head submodules (kept so DoRA can attach) --
        ...
        # -- Aggregator --
        # agg_type: "linear" | "attention" (sample-wise selection).
        if agg_type == "linear":
            # -- Linear aggregator --
            # Without correction ||pooled|| ~= (1 + block_scale * (K-1)) * ||h[:,0]||, and the
            # F.normalize Jacobian would then shrink the block_0 gradient. Dividing by
            # norm_factor restores the single-probe magnitude while keeping block_k != 0.
            ...
            #   logits[b, k] = (h[b, k] . agg_query) / attn_temp + agg_init_bias[k]
            #   weights[b]   = softmax(logits[b])
            #   pooled[b]    = agg_proj(sum_k weights[b, k] * h[b, k])
            # init: agg_query = probe[0], agg_init_bias = [+1, 0, ...], agg_proj = I
            #   so at init pooled ~= 0.73 * h[:,0] + 0.27 * h[:,1] and block_0 stays dominant.
            self.aggregator = None    # legacy linear disabled; kept as None for state_dict compatibility
            self.agg_query = nn.Parameter(torch.empty(E))
            with torch.no_grad():
                # init toward probe[0], the preserved clone
    ...
    def forward(self, hidden_state, attention_mask=None):
        """SigLIP2 head.forward compatible. Input (B, L, D) -> output (B, D), branching on agg_type."""
        B = hidden_state.shape[0]
        probe = self.probe.expand(B, -1, -1)  # (B, K, D)

        # the pooler is currently only called with mask=None; this guards against a transformers change
        ...
        if self.agg_type == "linear":
            ...
            # logits: (B, K) - h * agg_query / temp + agg_init_bias
...
def _replace_head_with_multi_probe(base_model, num_probes=None,
                                    ...
                                    agg_type=None,
                                    attn_temp=None):
    """Wrap vision_model.head in a MultiProbePoolingHead. Call order: extend_position_embedding
    -> _replace_pooler_mha_with_split -> here -> freeze_vision_encoder -> get_peft_model.
    """
    ...
    # equivalence check against the single-probe head at init (RNG forked for the dummy generator)
    ...
    try:
        with torch.random.fork_rng(devices=fork_devices), torch.no_grad():
            ...
            if old_dev.type == "cuda":
                torch.cuda.manual_seed(0)
            # patches = (IMAGE_SIZE // patch_size)^2; patch_size is read from the config
            try:
    ...
    if orig_out is not None and new_out is not None:
        ...
        # the attention aggregator is a separate path, so the numerical equality check is off
        if agg_type == "attention":
            ...
            if block_scale == 0:
                ...
                # expected max_diff ~= block_scale * (K-1) * orig_scale; a generous 5x margin, still warns
...
def _is_multi_probe_param(name: str) -> bool:
    """Multi-probe params (unified): probe + aggregator. With agg_type=attention the
    agg_query/agg_init_bias/agg_proj tensors join the same group.
    """
    ...
    return (
        ...
        # attention aggregator parameters
...
def _is_multi_probe_probe_param(name: str) -> bool:
    """The probe alone (the LR_MULTI_PROBE_PROBE group), excluding everything aggregator-side."""
    if "lora" in name.lower():
        return False
...
def _build_text_dora_patterns(num_text_layers: int, rank: int, alpha: int) -> tuple:
    ...
    MetaClip2 text uses the CLIP module names text_model.encoder.layers.{i}.self_attn.{q,k,v,out}_proj.
    """
...
def _verify_dora_ranks(peft_model, *, expected_text: int, expected_vision: int, mode: str):
    """Check that the actual lora_A rank of text/vision layer 0 matches the intent."""
    r_text, r_vis = None, None
    for name, module in peft_model.named_modules():
        if not name.endswith(".lora_A.default"):
            continue
        base_name = name[:-len(".lora_A.default")]
        # text: layer-0 attention query/value (XLM-V), falling back to q_proj
        if r_text is None and (
            ("text_model" in base_name) and (".0." in base_name) and
            ("query" in base_name or "value" in base_name or "q_proj" in base_name)
        ):
            r_text = module.weight.shape[0]
        # vision: layer-0 q_proj
        if r_vis is None and (
            ("vision_model" in base_name) and (".0." in base_name) and ("q_proj" in base_name)
        ):
            r_vis = module.weight.shape[0]
        if r_text is not None and r_vis is not None:
            break
    print(f"  [rank-verify] text   (layer 0) lora_A rank: {r_text}  (expected {expected_text})")
    print(f"  [rank-verify] vision (layer 0) lora_A rank: {r_vis}  (expected {expected_vision})")
    if r_vis is None:
        print("  [rank-verify] vision layer-0 q_proj lora_A not found - skipped")
    elif r_vis != expected_vision:
        raise AssertionError(f"vision rank mismatch: actual={r_vis}, expected={expected_vision}.")
    if r_text is None:
        print("  [rank-verify] text layer-0 lora_A not found - skipped")
    elif r_text != expected_text:
        raise AssertionError(
            f"text rank mismatch: actual={r_text}, expected={expected_text}. rank_pattern was not applied.")
    print(f"  [rank-verify] {mode} rank check passed (text r={expected_text}, vision r={expected_vision})")
...
def _unfreeze_pooler_dora(peft_model):
    ...
    Zero matches while FREEZE_POOLER_DORA=False raises, since a silent skip would lose the effect.
    """
    n_total = 0
    for name, p in peft_model.named_parameters():
...
def _unfreeze_multi_probe_params(peft_model) -> dict:
    ...
    Called after the vision freeze; both components must match or it raises.
    """
...
def _unfreeze_logit_params(peft_model):
    """Make logit_scale trainable (its own LR group). CLIP has no logit_bias.
    LOGIT_SCALE_FREEZE=True keeps it at its pretrained value instead.
    """
    for name, p in peft_model.named_parameters():
        if "logit_scale" in name:
            p.requires_grad_(not LOGIT_SCALE_FREEZE)
    if LOGIT_SCALE_FREEZE:
        print("  [logit] LOGIT_SCALE_FREEZE=True -> logit_scale frozen at init")
...
def _print_trainable_breakdown(c: dict):
    ...
    # no extra rows; with freeze=True every pretrained row is frozen
...
def _verify_model_invariants(peft_model, tokenizer):
    """Verify the invariants that must hold when build_model() returns: tokenizer class,
    DoRA wrap distribution, no non-LoRA trainable in the vision base, and the head type.
    """
    ...
    if NUM_PROBES > 1:
        ...
        # init multi-probe diagnostic: relative perturbation gives offdiag_sim ~= 0.9999
...
def build_model():
    """Build the MetaCLIP2 ViT-L/14 + DoRA + FLAIR model.

    Step 2 : vision encoder base permanently frozen
    Step 3 : DoRA wrap over the vision+text target union, text rank overridden per layer
    Step 4 : vision encoder DoRA wrapper trainable or not
    Step 6 : logit_scale trainable; there is no logit_bias
    Step 1 : text Position Interpolation 77 -> 128, applied before the DoRA wrap
    """
    ...
    # the SplitMHA / Multi-Probe surgery calls are removed; the definitions stay
    # text PI 77 -> 128, patching the forward before the DoRA wrap
    if POSITION_PI_ENABLED:
        print("\n[step 1] Text Position Interpolation 77→128 (§12) ...")
    ...
    # step 3: DoRA wrap over the vision + text module union, with a text rank override
    ...
    # MetaClip2 text is CLIP-style '.layers' (12), modules q/k/v/out_proj and fc1/fc2
    _txt_enc = base_model.text_model.encoder
    if hasattr(_txt_enc, "layers"):
        num_text_layers = len(_txt_enc.layers)
    elif hasattr(_txt_enc, "layer"):
        num_text_layers = len(_txt_enc.layer)
    else:
        num_text_layers = 12
        print("  [warn] text encoder layer container not found -> num_text_layers=12")
    ...
    dora_targets = list(dict.fromkeys(DORA_TARGETS_VISION + DORA_TARGETS_TEXT))  # union dedup; vision and text share the same suffixes
    dora_cfg = LoraConfig(
    ...
    # step 4: vision DoRA wrapper trainable or not
    ...
    # the pooler/multi-probe unfreeze is removed; no surgery is applied

    print("\n[step 5] text trainable = DoRA only ...")
    _unfreeze_logit_params(peft_model)

    # step 7: trainable counts per LR group
    _print_trainable_breakdown(_count_trainable_breakdown(peft_model))
    _verify_model_invariants(peft_model, tokenizer)

    # the lowercase tokenizer wrapper is removed; raw AutoTokenizer
    return peft_model, tokenizer

def get_logit_params(model):
    """Handle on logit_scale (its own LR group). CLIP has no logit_bias."""
    base = model.get_base_model() if hasattr(model, "get_base_model") else model
    out = []
...
# =========================================================
# 7. Loss - SigLIP sigmoid pair loss
# =========================================================
def sigmoid_pair_loss(v, t, image_ids,
    ...
    """Unified sigmoid pair loss (multi-positive plus an optional soft label):
    loss[i, j] = -[p[i, j] * logsigmoid(x) + (1 - p[i, j]) * logsigmoid(-x)].
    """
...
# ----- Probe Diversity Regularization -----
def probe_diversity_loss(model) -> torch.Tensor:
    """Probe off-diagonal cosine similarity penalty:
    L_div = mean_{i!=j} (probe_i . probe_j / (||probe_i|| ||probe_j||))^2.
    Returns zero at K=1 or when there is no multi-probe head.
    """
    base = model.get_base_model() if hasattr(model, "get_base_model") else model
    try:
        head = base.vision_model.head
...
# ----- Loss dispatch (USE_SOFT_LABEL on/off) -----

# default factory for the per-epoch soft label diagnostic accumulator
...
# ----- uniformity regularization (Wang & Isola 2020) -----
def uniformity_loss(x, t: float = 2.0) -> torch.Tensor:
    """Wang & Isola (2020) uniformity: log E_{i!=j}[ exp(-t * ||x_i - x_j||^2) ].
    x is (N, D) and is L2-normalized inside; only off-diagonal pairs are used.
    Lower (more negative) means the representation spreads more evenly over the sphere.
    """
    if x.shape[0] < 2:
...
# ----- cross-GPU negative all-gather -----
def _gather_features_and_ids(v, t, image_ids):
    """Widen the negatives to the whole global batch by all-gathering v, t and image_ids.
    Only the local rank keeps grad; image_ids get a rank offset before the gather.
    """
    if not (dist.is_available() and dist.is_initialized()):
        return v, t, image_ids
    ws = get_world_size()
    if ws <= 1:
        return v, t, image_ids
    rank = get_rank()
    n_local = v.shape[0]                            # images per rank (DROP_LAST + a uniform sampler make every rank equal)

    image_ids_offset = image_ids + rank * n_local   # local image_ids -> global index space

    gv  = [torch.zeros_like(v) for _ in range(ws)]
    gt  = [torch.zeros_like(t) for _ in range(ws)]
    gid = [torch.zeros_like(image_ids_offset) for _ in range(ws)]
    dist.all_gather(gv, v.contiguous())
    dist.all_gather(gt, t.contiguous())
    dist.all_gather(gid, image_ids_offset.contiguous())
    # all_gather detaches, so put the original (grad-carrying) tensor back in the local slot
    gv[rank] = v
    gt[rank] = t
    return torch.cat(gv, dim=0), torch.cat(gt, dim=0), torch.cat(gid, dim=0)

# ----- symmetric InfoNCE (CLIP); replaces sigmoid_pair_loss -----
def clip_infonce_loss(v, t, image_ids, logit_scale):
    """symmetric InfoNCE (single-GPU reference path). v is (Ni, D), t is (Nt, D) and
    image_ids maps each text to its image. Multi-positive is handled by an i2t soft mask.
    """
    v = F.normalize(v.float(), dim=-1)
    t = F.normalize(t.float(), dim=-1)
    scale = logit_scale.exp().clamp(max=LOGIT_SCALE_MAX)
    logits_t2i = scale * (t @ v.t())                 # (Nt, Ni)
    loss_t2i = F.cross_entropy(logits_t2i, image_ids)
    Ni = v.shape[0]
    pos = (image_ids.unsqueeze(0) == torch.arange(Ni, device=v.device).unsqueeze(1)).float()  # (Ni, Nt)
    pos = pos / pos.sum(dim=1, keepdim=True).clamp(min=1.0)
    loss_i2t = -(pos * F.log_softmax(logits_t2i.t(), dim=1)).sum(dim=1).mean()
    return 0.5 * (loss_t2i + loss_i2t)

def clip_infonce_loss_gathered(v, t, image_ids, logit_scale):
    """section 3.3: open_clip local-loss style. local text/image rows scored vs global pool.
      world_size==1 -> gather no-op -> reduces to clip_infonce_loss.
    """
    v_all, t_all, ids_all = _gather_features_and_ids(v, t, image_ids)
    if v_all is v and t_all is t:
        # no-gather (single-GPU) -> reference path
        return clip_infonce_loss(v, t, image_ids, logit_scale)
    v_l = F.normalize(v.float(), dim=-1)
    t_l = F.normalize(t.float(), dim=-1)
    v_g = F.normalize(v_all.float(), dim=-1)
    t_g = F.normalize(t_all.float(), dim=-1)
    scale = logit_scale.exp().clamp(max=LOGIT_SCALE_MAX)
    n_local = v.shape[0]
    ids_local_global = image_ids + get_rank() * n_local           # local text -> global image index
    # text->image: local text rows vs global image pool
    logits_t2i = scale * (t_l @ v_g.t())                          # (Nt_local, Ni_global)
    loss_t2i   = F.cross_entropy(logits_t2i, ids_local_global)
    # image->text: local image rows vs global text pool (K-positive soft mask)
    Ni_global = v_g.shape[0]
    img_global_idx = torch.arange(n_local, device=v.device) + get_rank() * n_local   # local img -> global img idx
    logits_i2t = scale * (v_l @ t_g.t())                          # (Ni_local, Nt_global)
    pos = (ids_all.unsqueeze(0) == img_global_idx.unsqueeze(1)).float()  # (Ni_local, Nt_global)
    pos = pos / pos.sum(dim=1, keepdim=True).clamp(min=1.0)
    loss_i2t = -(pos * F.log_softmax(logits_i2t, dim=1)).sum(dim=1).mean()
    return 0.5 * (loss_t2i + loss_i2t)
...
def compute_step_loss(v, t, image_ids,
                      ...
                      *, logit_scale, logit_bias=None,
                      want_soft_diag: bool,
                      row_normalizer: int = None):
    """Call symmetric InfoNCE (CLIP), with cross-GPU gather support.
    The soft-label / logit_bias / row_normalizer arguments are unused on this path.
    """
    if CROSS_GPU_NEG_GATHER:
        loss = clip_infonce_loss_gathered(v, t, image_ids, logit_scale)
    else:
        loss = clip_infonce_loss(v, t, image_ids, logit_scale)
    return loss, None

# =========================================================
# Halt exception hierarchy
# =========================================================
# Every abort reason is a TrainHalt subclass, so the single try/except in main sorts them
# and the subtype lands in halt_reason in summary.json.
...
def _check_loss_safety(loss_val: float, global_step: int, threshold_scale: float = 1.0, nan_val=None):
    ...
    threshold_scale accounts for the loss matrix growing with world_size under cross-GPU gather.
    """
    eff_threshold = LOSS_EXPLOSION_THRESHOLD * max(1.0, threshold_scale)
    # explosion is judged on loss_main and NaN/Inf on the combined value - looking only at
    #   loss_main would miss a NaN coming from the FILIP or uniformity term
...
def perturb_caption(text: str, mode: str, seed=None) -> str:
    """Rule-based caption perturbation. Only the modes named in CAPTION_AUGMENT_PROBS:
    original (unchanged) and order (shuffle sentences split on '. ').
    """
    if not text:
        return text
    if mode == "original":
        return text
    if mode == "order":
        rng = random.Random(seed + stable_hash(text) % 10**6) if seed is not None else random
...
def augment_caption_train(text: str) -> str:
    """Apply a random perturbation to a caption during training (a different view per call)."""
    if not CAPTION_AUGMENT_TRAIN:
        return text
    modes = list(CAPTION_AUGMENT_PROBS.keys())
    probs = list(CAPTION_AUGMENT_PROBS.values())
    mode = random.choices(modes, weights=probs, k=1)[0]
    return perturb_caption(text, mode, seed=None)
...
class ParamEMA:
    ...
    update() blends, apply_shadow() swaps the EMA weights in for inference, restore() puts back.
    """
...
def _partition_trainable_params(model) -> dict:
    ...
    The probe and aggregator split into their own groups when LR_MULTI_PROBE_PROBE_SPLIT=True.
    """
    groups = {
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if "flair_pooler" in name:        # the FLAIR pooler gets its own group and LR
            groups["flair"].append(p)
            continue
        if "finegrain_head" in name:      # the fine-grained head gets its own group and LR
...
def build_optimizer(model, lr_dora: float = None, lr_scale: float = 1.0):
    """AdamW with separate LR groups: position_emb (text only, WD=0), dora (text + vision encoder
    + pooler), multi_probe (probe and aggregator, optionally split), logit, and the new heads.
    """
    ...
    group_specs = [
        ...
        ("logit",              LR_LOGIT,                0.0),
        ("finegrain",          LR_FINEGRAIN,            WEIGHT_DECAY),   # fine-grained projection head
        ("flair",              LR_FLAIR,                WEIGHT_DECAY),   # FLAIR pooler
    ]
    param_groups = [
...
def build_scheduler(optim, total_steps, warmup_steps: int):
    """LambdaLR cosine with warmup and a horizon clamp:
    below warmup a linear ramp, then a cosine decay to eta_min, then flat at eta_min.
    """
...
def build_ckpt_extra(epoch, **overrides) -> dict:
    """The common fields of the save_checkpoint extra dict (model_name / dora / seed /
    pooler_mha_split) plus the epoch, merged with the caller's overrides.
    """
    base = {
        "epoch":            epoch,
        ...
        # Position Interpolation marker, for the inference-side surgery check
...
def _collect_logit_and_position(base, extras: dict):
    """Collect logit_scale and the text position_embedding into extras. CLIP has no logit_bias."""
    if hasattr(base, "logit_scale") and isinstance(base.logit_scale, nn.Parameter):
...
def _collect_multi_probe_state(head, extras: dict):
    ...
    Skipped silently when the head is not multi-probe.
    """
...
def _pack_extras_state(base) -> dict:
    ...
    # FineGrainedHead lives outside the PEFT adapter, so it is saved manually
    fg = getattr(base, "finegrain_head", None)
    if fg is not None:
        extras["finegrain_head"] = {k: v.detach().cpu() for k, v in fg.state_dict().items()}
    # FLAIR pooler lives outside the PEFT adapter, so it is saved manually
...
def save_checkpoint(model, ckpt_dir: str, tag: str, extra: dict = None):
    ...
    The pretrained position rows are verified unchanged right before writing.
    """
...
# =========================================================
# 10.1 Resume - training state lives only in last/
#      (optimizer/scheduler/scaler/EMA/RNG/counters; the weights come from save_checkpoint)
# =========================================================
def _serialize_ema_shadow(ema):
    """ParamEMA.shadow (id-keyed) -> a list[dict(shape, tensor)] in ema.params order.
    The shape is stored so a changed param order or shape fails fast on restore.
    """
    if ema is None:
        return None
    return [
...
def save_resume_state(*, ckpt_dir: str, epoch: int, global_step: int,
    ...
    The weights come from save_checkpoint.
    """
...
def _resolve_resume_ckpt_dir(arg: str) -> str:
    ...
    A run directory, its checkpoints/last, or the last directory itself are all accepted.
    """
    if not arg:
        return ""
    ...
    if os.path.basename(p) == "last" and os.path.isfile(os.path.join(p, "resume_state.pt")):
        return p
    if os.path.isdir(p):
        # the argument may already be the last/ directory itself
...
def load_model_weights_from_ckpt(peft_model, ckpt_path: str):
    ...
    # CLIP has no logit_bias, so the restore branch is dropped
...
def print_run_banner(run_name: str, run_dir: str):
    ...
    print(f"  Schedule: horizon={SCHEDULE_HORIZON_EPOCHS}ep cosine, "
          f"hard stop at ep{EPOCHS} (no early stop)")
    print(f"  EMA: enabled={EMA_ENABLED}, half_life={EMA_HALF_LIFE_EPOCHS}ep, start_ep={EMA_START_EPOCH}")
    print(f"  Soft Label: USE={USE_SOFT_LABEL} (v19_8: off → vanilla multi-pos SigLIP)")
    print(f"  [v19_8] logit_scale clamp: enabled={LOGIT_SCALE_CLAMP_ENABLED} max_exp={LOGIT_SCALE_MAX} "
          f"freeze={LOGIT_SCALE_FREEZE}")
    print(f"  [v19_8] Cross-GPU neg gather: {CROSS_GPU_NEG_GATHER} (active iff world_size>1)")
    print(f"  [v19_8] Uniformity reg: enabled={UNIFORMITY_REG_ENABLED} "
          f"λ={UNIFORMITY_REG_LAMBDA} t={UNIFORMITY_REG_T} (rank-local v,t)")
    print(f"  Caption Aug: train={CAPTION_AUGMENT_TRAIN} "
          f"probs={CAPTION_AUGMENT_PROBS}")
    ...
    print(f"  Data: manifest={MANIFEST_TAG} unique_image_mode={USE_UNIQUE_IMAGE_MODE} "
          f"subset_mode={SUBSET_MODE}")
    print(f"  Safety: loss_explosion>{LOSS_EXPLOSION_THRESHOLD}, NaN_abort={LOSS_NAN_ABORT}")
    print(f"  [v23] FLAIR: enabled={FLAIR_ENABLED} λ={FLAIR_WEIGHT} dim={FLAIR_DIM} "
...
def _log_train_step(*, metric_writer, tb,
    ...
    rec = {
        ...
        # record the split LR group values too (0 when inactive)
...
def _diagnose_caption_lengths(train_rows, sample_size: int = 10000):
    """Report the sentence-count distribution of the captions, which bounds how much the
    order augmentation can do. Sampled at random to avoid a first-N bias.
    """
    n = min(sample_size, len(train_rows))
    indices = random.Random(0).sample(range(len(train_rows)), n)
    def _cap_text(r):
        # image-level records use captions (list); the old format used caption (str)
        caps = r.get("captions")
    ...
    if pct1 > 30:
        print(f"  [warn] {pct1:.1f}% of captions are single-sentence, so order mode (split on '. ') is "
              f"a no-op for that share. The effective order rate is below 0.05.")
...
def _verify_tokenizer_pickleable(tokenizer):
    """tokenizer pickle round trip. The __getattr__/__setattr__ override can fail silently,
    so this runs before the DataLoader workers are spawned.
    """
    import pickle
    try:
...
def ddp_broadcast_obj(obj, src: int = 0):
    """Broadcast a python object from rank=src to every rank."""
    if get_world_size() == 1:
        return obj
    objs = [obj]
...
def silence_non_main_stdout():
    """Redirect stdout/stderr of rank != 0 to /dev/null so the log is not duplicated."""
    if is_main():
        return
    f = open(os.devnull, "w")
    sys.stdout = f
...
# ===========================================================================
# 2. DistributedSamplerWrapper - partition a WeightedRandomSampler for DDP
# ===========================================================================
class _IndicesDataset:
    """Disguise a fixed-length index list as a Dataset (DistributedSampler only uses len and [])."""
...
class DistributedSamplerWrapper(DistributedSampler):
    """Partition the per-epoch index list of a Sampler (e.g. WeightedRandomSampler) across ranks
    so each rank sees 1/world_size of it. Call .set_epoch(epoch) before every epoch.
    """

    def __init__(self, sampler: Sampler, num_replicas: int, rank: int,
                 base_seed: int = 0, drop_last: bool = True):
        self.sampler = sampler
        self._base_seed = base_seed
        # materialize once for epoch 0
        indices = list(sampler)
        self._ds = _IndicesDataset(indices)
        super().__init__(
            dataset=self._ds,
            num_replicas=num_replicas,
            rank=rank,
            shuffle=False,          # the underlying sampler is already stochastic
            drop_last=drop_last,
        )

    def set_epoch(self, epoch: int):
        super().set_epoch(epoch)
        gen = getattr(self.sampler, "generator", None)
        if gen is not None:
            gen.manual_seed(self._base_seed + epoch * 1000003)
        indices = list(self.sampler)
        self._ds = _IndicesDataset(indices)
        # refresh the DistributedSampler's own dataset/num_samples
        self.dataset = self._ds
        if self.drop_last:
            self.num_samples = len(self._ds) // self.num_replicas
        else:
            self.num_samples = math.ceil(len(self._ds) / self.num_replicas)
        self.total_size = self.num_samples * self.num_replicas

    def __iter__(self):
        sub_indices = list(super().__iter__())
        return iter(self._ds[i] for i in sub_indices)

    def __len__(self):
        return self.num_samples

# ===========================================================================
# 3. Joint encoder - one forward for both sub-encoders (required by DDP)
#    plus the fine-grained head and the FILIP-style late-interaction loss
# ===========================================================================
class FineGrainedHead(nn.Module):
    """Project token/patch hidden states into the shared late-interaction space (L2-normalized).
    SigLIP only aligns the pooled vectors, so the raw token/patch spaces need this projection.
    """
...
def _fg_patch_pool(fg_patch, patch_pool):
    ...
    if g * g != Lp:
        ...
        #   so a violated patch-grid assumption fails fast
        raise ValueError(
...
class TextConditionedPooler(nn.Module):
    ...
    The pooled text is the query and the vision patches are K/V; the cross-attended
    vector is then scored against the projected text.
    """
    def __init__(self, tok_dim, patch_dim, fdim=FLAIR_DIM, num_heads=FLAIR_NUM_HEADS,
        ...
        # raw hidden norm spread makes attention peaky at init, so LayerNorm goes before projection
        ...
        self.q_proj   = nn.Linear(tok_dim,   fdim)   # text query
        self.k_proj   = nn.Linear(patch_dim, fdim)   # vision key
        self.v_proj   = nn.Linear(patch_dim, fdim)   # vision value
        self.out_proj = nn.Linear(fdim, fdim)
        self.t_proj   = nn.Linear(tok_dim, fdim)     # text-side scoring projection
    ...
    def forward(self, t_pooled, v_loc):
        ...
        Ni, Lp, _ = v_loc.shape
        H, Dh = self.h, self.dh
        # called inside autocast(bf16); force fp32 for a stable softmax/normalize
...
def finegrain_filip_loss(fg_tok, fg_patch, keypad, image_ids,
    ...
    S[i,j] = mean over the real tokens of text i of the top-k mean over the patches of image j.
    """
    fg_patch = _fg_patch_pool(fg_patch, patch_pool)     # same downsample in training and inference
    # ---- token x patch sim -> top-k mean over patches -> mean over real tokens ----
    # (Nt,Lt,Ni,Lp): memory = Nt*Lt*Ni*Lp, so patch_pool shrinks Lp
    sim = torch.einsum('itd,jpd->itjp', fg_tok, fg_patch)
    # top-k mean over patches (k = round(Lp * ratio)); softer than a plain max
    ...
    # FILIP in-batch retrieval acc - a fine-grained signal pooled hard_margin cannot show
    with torch.no_grad():
...
def flair_tcs_loss(S, image_ids, logit_scale=FLAIR_LOGIT_SCALE, symmetric=FLAIR_SYMMETRIC):
    ...
    S is an asymmetric (Nt,Ni) score; text->image is CE(logit_scale*S, image_ids) and
    symmetric=True adds the image->text direction over the same matrix.
    """
...
class _DDPJointEncoder(nn.Module):
    """Encode image and text with the base model in a single forward.
    DDP only reduces gradients for params used inside one forward, so the fine-grained head
    must run here too.
    """
    ...
    def _encode(self, pixel_values, input_ids, attention_mask):
        """One vision/text forward plus a manual projection (CLIP). pooler_output is pre-projection,
        so the global loss must use the projected features; get_*_features would cost a second forward.
        """
        vo = self.base.vision_model(pixel_values=pixel_values)       # last_hidden_state + pooler_output
        to = self.base.text_model(input_ids=input_ids, attention_mask=attention_mask)
        v = self.base.visual_projection(vo.pooler_output)            # projected global image feat
        t = self.base.text_projection(to.pooler_output)             # projected global text feat
        return v, t, vo, to

    def forward(self, pixel_values, input_ids, attention_mask):
        # ---- FLAIR path (train-only pooler) ----
        if self.flair_enabled:
            v, t, vo, to = self._encode(pixel_values, input_ids, attention_mask)
            v_loc = vo.last_hidden_state                            # unprojected local tokens
            if FLAIR_STRIP_CLS:
                v_loc = v_loc[:, 1:, :]                             # drop CLS -> patch grid (256)
            # the pooler's tok_dim is the text hidden_size, so the query is unprojected text too,
            #   which keeps it consistent with the unprojected vision side and fixes the dim contract
            S = self._flair_pooler(to.pooler_output, v_loc)         # (Nt,Ni)
            return v, t, {"flair_S": S}
        # ---- FILIP path (FINEGRAIN_ENABLED=False; kept for structure) ----
        if self.fg_enabled:
            v, t, vo, to = self._encode(pixel_values, input_ids, attention_mask)
            v_loc = vo.last_hidden_state
            if FLAIR_STRIP_CLS:
                v_loc = v_loc[:, 1:, :]
            fg_tok, fg_patch = self._fg_head(to.last_hidden_state, v_loc)
        ...
        # ---- global only ----
        v, t, _vo, _to = self._encode(pixel_values, input_ids, attention_mask)
        return v, t, {}

def unwrap_base(model: nn.Module):
    """Strip the DDP/JointEncoder wrapper and return the base model."""
    m = model
    if isinstance(m, DDP):
        m = m.module
    if isinstance(m, _DDPJointEncoder):
        m = m.base
    return m

# ===========================================================================
# 4. DDP build_train_loader - same as build_train_loader plus the sampler wrap
# ===========================================================================
def build_train_loader_ddp(train_rows, tokenizer, train_tf,
                           world_size: int, rank: int):
    """Same dataset/collate/sampler as build_train_loader, with the sampler wrapped in
    DistributedSamplerWrapper.
    """
    train_ds = PABDataset(
        train_rows, train_tf, is_training=True,
        unique_image_mode=USE_UNIQUE_IMAGE_MODE,
        n_positives=N_POSITIVES_PER_IMAGE,
    )
    collate = make_collate(
        tokenizer, MAX_TEXT_LENGTH,
        n_positives=N_POSITIVES_PER_IMAGE,
    )
    if is_main():
        print(f"  positives per image: K={N_POSITIVES_PER_IMAGE} "
    ...
    sampler_gen.manual_seed(SEED)
    sampler_rows = train_ds.unique_rows if USE_UNIQUE_IMAGE_MODE else train_rows
    inner_sampler, label_count, action_count = build_train_sampler(
        sampler_rows, sampler_gen)

    if inner_sampler is None:
        # shuffle path: plain DistributedSampler
        ...
        ddp_sampler = DistributedSamplerWrapper(
            inner_sampler,
            num_replicas=world_size,
            rank=rank,
            base_seed=SEED,
    ...
    loader = DataLoader(
        ...
        drop_last=DROP_LAST,
        worker_init_fn=worker_init_fn if NUM_WORKERS > 0 else None,
        # collate is a module-level _TrainCollator instance, so it pickles under spawn
    )
    return loader, label_count, action_count

# ===========================================================================
# 5. main_ddp - the training flow of main(), reworked for DDP
# ===========================================================================
def main_ddp(local_rank: int, world_size: int, physical_gpu: int,
    ...
    # silence rank != 0 so the run dir and logger do not collide
    silence_non_main_stdout()

    # ----- seed: shifted per rank for sampler diversity, SEED itself stays fixed -----
    set_seed(SEED + 13 * local_rank)
    ensure_global_dirs()

    # ----- run dir: rank 0 decides, then broadcasts -----
    if is_main():
        run_dir, run_name = make_run_dir(extra_run_note)
        paths = _setup_run_paths(run_dir)
    else:
        run_dir, run_name, paths = None, None, None
    run_dir = ddp_broadcast_obj(run_dir, src=0)
    run_name = ddp_broadcast_obj(run_name, src=0)
    paths = ddp_broadcast_obj(paths, src=0)
    ckpt_dir = paths["ckpt"]
    tb_dir = paths["tb"]
    ...
    if is_main():
        ...
        print(f"  [DDP] world_size={world_size}  rank={get_rank()}  "
    ...
    summary = {
        ...
        "ddp_gpus": str(physical_gpu),
        "resumed_from": resume_path or None,         # provenance: which ckpt this continued from
    }
    # best-ckpt selection happens outside training, so best_* stays inert (-1 = not selected)
    best_score = -1.0
    best_epoch = -1
    best_metrics = {}
    global_step = 0
    ...
    resuming = bool(resume_path)             # whether this is a resume

    try:
        # ---- (1) Manifest ---------------------------------------------------------
        if is_main():
            print(f"\n[1/5] Manifest — {MANIFEST_PATH}")
        rows = load_manifest()
        if is_main():
        ...
        # fine-grained head attach - after the freeze in build_model, so it stays trainable
        #   attach to the peft model so build_optimizer and the DDP wrap both pick it up
        if FINEGRAIN_ENABLED:
            ...
            # attach to base so save/restore go through unwrap_base and _pack_extras_state,
            #   while the optimizer still sees it as "finegrain_head"
            ...
            if is_main():
                ...
                print(f"  [v23] FineGrainedHead attached (tok={_fg_dim}/pat={_pat_dim}→{FINEGRAIN_DIM}, "
        ...
        if FLAIR_ENABLED:
            ...
            if is_main():
                ...
                print(f"  [v23] TextConditionedPooler attached "
        ...
        # ----- resume: model weights first; the optimizer is restored after it is built -----
        if resuming:
            if is_main():
        ...
        train_loader, label_count, action_count = build_train_loader_ddp(
            train_rows, tokenizer, train_tf,
            world_size=world_size, rank=get_rank(),
        )
        ...
        # ---- DDP LR auto-scaling (v18_4_L parity) ----
        ws_ratio = max(1, world_size) / max(1, DDP_LR_REF_WORLD_SIZE)
        if DDP_LR_SCALING == "sqrt":
            lr_scale = math.sqrt(ws_ratio)
        elif DDP_LR_SCALING == "linear":
            lr_scale = float(ws_ratio)
        elif DDP_LR_SCALING == "none":
            lr_scale = 1.0
        else:
            raise ValueError(f"DDP_LR_SCALING={DDP_LR_SCALING!r} (expected sqrt|linear|none)")
        ...
        if is_main():
            ...
            print(f"  per-rank batch      = {BATCH_SIZE}  "
                  f"× world_size={world_size}  → global batch = "
            ...
            print(f"  DDP_LR_SCALING         = {DDP_LR_SCALING}  "
                  f"(ref world_size={DDP_LR_REF_WORLD_SIZE})")
            print(f"  lr_scale               = {lr_scale:.4f}  "
                  f"(world_size/ref = {ws_ratio:g})")
            print(f"  LR_DoRA (base→eff)     = {LR:.2e} → {LR*lr_scale:.2e}")
            print(f"  LR_min_ratio           = {LR_MIN_RATIO}")
            summary["lr_scale"] = lr_scale
            summary["ddp_lr_scaling"] = DDP_LR_SCALING
            summary["ddp_lr_ref_world_size"] = DDP_LR_REF_WORLD_SIZE
            summary["effective_lrs"] = {
                g.get("name", f"g{i}"): float(g["lr"])
                for i, g in enumerate(optimizer.param_groups)
            }

        # ---- (4.5) DDP wrap (after the optimizer is built) ----
        # JointEncoder merges the image and text forward into one, then DDP wraps it
        _pad_id = getattr(tokenizer, "pad_token_id", None)
        # FILIP key padding needs pad_token_id; None would make pad tokens count as real and
        #   corrupt the supervision, so this fails fast
        ...
        if world_size > 1:
            ddp_model = DDP(
                joint,
                device_ids=[physical_gpu],
                output_device=physical_gpu,
                broadcast_buffers=False,           # every buffer is guaranteed the same init
                find_unused_parameters=False,      # every trainable is used in the forward
                gradient_as_bucket_view=True,
                static_graph=False,                # the multi-probe head may take a dynamic path
            )
        else:
            ddp_model = joint
        ...
        if is_main():
            print("\n" + "-" * 64)
            print(f"  Training  (DDP world_size={world_size})")
            print("-" * 64)

        # split logit from the rest
        unwrapped = unwrap_base(ddp_model)
        ...
        # ---- resume: optimizer / scheduler / scaler / EMA / RNG / counters ----
        if resuming:
            if is_main():
            ...
            ddp_barrier()
            # broadcast so every rank holds the same counters (RNG is handled separately)
            sync = ddp_broadcast_obj({
                "start_epoch": start_epoch, "global_step": global_step,
                "best_score": best_score, "best_epoch": best_epoch,
                "best_metrics": best_metrics, "early_stop_counter": early_stop_counter,
                "last_reset_score": last_reset_score,
            }, src=0)
            start_epoch        = sync["start_epoch"]
            global_step        = sync["global_step"]
            best_score         = sync["best_score"]
            best_epoch         = sync["best_epoch"]
            best_metrics       = sync["best_metrics"]
            early_stop_counter = sync["early_stop_counter"]
            last_reset_score   = sync["last_reset_score"]

            # keep DDP RNG diversity: resume_state.pt holds rank 0's RNG, and starting every rank
            # from it would lose per-rank sampler/aug diversity. rank 0 keeps the loaded RNG for exact
            # reproducibility; the others re-apply their rank shift.
            if local_rank != 0:
                set_seed(SEED + 13 * local_rank + int(global_step))
        ...
        for epoch in range(start_epoch, EPOCHS + 1):
            ...
            # reseed the DDP sampler (per-epoch distribution and partition shuffle)
            ...
            for step, batch in enumerate(loop, 1):
                ...
                with torch.amp.autocast("cuda", dtype=AMP_DTYPE, enabled=USE_AMP):
                    # a single forward, which is what guarantees DDP gradient sync
                    v, t, aux = ddp_model(pixel_values, input_ids, attention_mask)
                    ...
                    # ---- Cross-GPU negative all-gather ----
                    # The gather itself happens inside compute_step_loss; only the global dimensions are
                    # derived here, for the sanity print and the threshold scale.
                    n_local_txt = t.shape[0]
                    # gather sanity print (step 1, rank 0 only).
                    # The collective must not run inside is_main() - only rank 0 would enter and the
                    # mismatch would deadlock NCCL. The global size is arithmetic: n_local * world_size.
                    if step == 1 and is_main():
                        if CROSS_GPU_NEG_GATHER and world_size > 1:
                            _gt, _gi = n_local_txt * world_size, v.shape[0] * world_size
                        else:
                            _gt, _gi = n_local_txt, v.shape[0]
                        print(f"  [gather-sanity] local_txt={n_local_txt} global_txt={_gt} "
                              f"global_img={_gi} (InfoNCE CE row-mean, no ws² blow-up)")

                    # symmetric InfoNCE; local v,t,image_ids go in and the gather happens inside
                    ...
                    # ---- uniformity regularization (off by default) ----
                    # computed on rank-local v,t, not the gathered tensors
                    ...
                    # ---- fine-grained FILIP token<->patch loss (local, in-batch) ----
                    #   local image_ids; the patches are avgpool-downsampled inside
                    ...
                    # ---- FLAIR text-conditioned pooling loss (local, in-batch) ----
                    _lam = FLAIR_WEIGHT   # default; guards against a _lam NameError in the mode-aware log
                    if FLAIR_ENABLED and flair_S is not None:
                        loss_flair, fg_acc = flair_tcs_loss(flair_S, image_ids)
                        _W = FLAIR_WEIGHT_WARMUP_EPOCHS          # lambda warmup, protecting the randomly initialised pooler
                ...
                if _FG_OR_FLAIR and is_main() and (step == 1 or step % LOG_EVERY_STEPS == 0):
                    _tag = "v23-flair" if FLAIR_ENABLED else "v23-fg"
                ...
                if FINEGRAIN_ENABLED and is_main() and step == 1:
                    # log the FILIP einsum dims and peak so a K or batch change shows up before it OOMs
                    ...
                    print(f"    [v23-fg-mem] sim=(Nt{_Nt},Lt{_Lt},Ni{_Ni},Lp{_Lp})={_elem/1e6:.0f}M "
                          f"(~{_elem*4/1024**3:.2f}GB fp32)  fwd-peak={_peak:.1f}GB (backward excluded)", flush=True)

                if FLAIR_ENABLED and is_main() and step == 1 and torch.cuda.is_available():
                    print(f"    [v23-flair-mem] S=(Nt{flair_S.shape[0]},Ni{flair_S.shape[1]}) "
                          f"fwd-peak={torch.cuda.max_memory_allocated()/1024**3:.1f}GB", flush=True)

                loss_val = loss.item()
                # InfoNCE (CE) is row-mean, so there is no world_size^2 blow-up: threshold_scale stays 1.0
                _loss_thresh_scale = 1.0
                # collapse detection uses loss_main, not the combined value - the FILIP and uniformity
                #   terms would otherwise eat the margin or hide a collapse
                ...
                if FINEGRAIN_ENABLED and is_main() and step == 1 and torch.cuda.is_available():
                    print(f"    [v23-fg-mem] post-backward peak="
                          f"{torch.cuda.max_memory_allocated()/1024**3:.1f}GB (exact OOM margin)", flush=True)

                if FLAIR_ENABLED and is_main() and step == 1 and torch.cuda.is_available():
                    print(f"    [v23-flair-mem] post-backward peak="
                          f"{torch.cuda.max_memory_allocated()/1024**3:.1f}GB (exact OOM margin)", flush=True)

                # verify after backward so the eval toggle cannot enter the GC recompute graph and the
                #   post-backward peak is not polluted by the four verify forwards
                if _FG_OR_FLAIR and is_main() and step == 1:
                    # In CLIP, pooler_output (pre-proj) != get_*_features (post-proj), so check that the
                    #   manual projection agrees with get_*_features - the same path training uses for v,t.
                    #   Compared in eval mode (dropout off); a warning, not an assert.
                    _was_train = unwrapped.training
                    unwrapped.eval()
                    try:
                        with torch.no_grad(), torch.amp.autocast("cuda", dtype=AMP_DTYPE, enabled=USE_AMP):
                            _vo = unwrapped.vision_model(pixel_values=pixel_values)
                            _to = unwrapped.text_model(
                                input_ids=input_ids, attention_mask=attention_mask)
                            _vp = _to_tensor(unwrapped.visual_projection(_vo.pooler_output))
                            _tp = _to_tensor(unwrapped.text_projection(_to.pooler_output))
                        ...
                        print(f"    [v23-verify] manual-proj vs get_*_features (eval) max|diff| "
                              f"v={_dv:.2e} t={_dt:.2e} [{_tag}]", flush=True)
                    except Exception as _e:
                        print(f"    [v23-verify] skipped: {type(_e).__name__}: {_e}", flush=True)
                    finally:
                ...
                # ---- gradient-spike rejection (every rank decides identically) ----
                if GRAD_SPIKE_SKIP and world_size > 1 and dist.is_initialized():
                    _gn_t = torch.tensor([grad_norm_total], device=device)
                    dist.all_reduce(_gn_t, op=dist.ReduceOp.MAX)
                    grad_norm_total = float(_gn_t.item())
                ...
                if will_log and is_main():
                    now = time.time()
                    samples_per_sec = (LOG_EVERY_STEPS * BATCH_SIZE * world_size) \
                    ...
                    diag["loss_uni"] = loss_uni_val   # uniformity reg (off -> 0.0)
            ...
            # ---- training progress metrics (hard_margin and friends) ----
            if is_main():
                ...
                # FLAIR only: early detection of pooled v,t corruption (last batch, trend only)
                _flair_nn = {}
                if FLAIR_ENABLED and is_main():
                    with torch.no_grad():
                        ...
                        print(f"  [v23-flair] pooled nn_sim  v={_vs:.4f}  t={_ts:.4f}  "
                ...
                metric_writer.log(
                    ...
                    fg_inbatch_acc_local=_fgacc,   # FILIP local in-batch retrieval acc (no gather, so trend only)
            ...
            # ---- rolling ckpt (rank0 only) ----------------------------------------
            if KEEP_LAST_N_CKPT != 0 and is_main():
                ...
                save_checkpoint(
                    ...
                    extra=build_ckpt_extra(epoch, ddp_world_size=world_size,
            ...
            # ---- per-epoch last ckpt + resume state (rank 0 only) ----
            # last/ is refreshed every epoch so an interrupted run resumes from the next one
            if is_main():
                try:
                    save_checkpoint(
                        unwrapped, ckpt_dir, "last",
                        extra=build_ckpt_extra(
                            epoch, ddp_world_size=world_size,
                            resume_enabled=True,
                        ),
                    )
                    save_resume_state(
        ...
        if is_main():
            save_checkpoint(
                unwrap_base(ddp_model), ckpt_dir, "last",
                extra=build_ckpt_extra(
                    final_epoch_meta,
        ...
        if is_main():
            if summary.get("status") != "completed":
                try:
                    _locals = locals()
                    if "ddp_model" in _locals and "ckpt_dir" in _locals:
                        save_checkpoint(
                            unwrap_base(_locals["ddp_model"]), ckpt_dir, "last",
                            extra=build_ckpt_extra(
                                _locals.get("epoch", 0),
                        ...
                        # save the resume state too, so a crash mid-epoch is still resumable
...
# ===========================================================================
def _free_port() -> int:
    """Pick one free TCP port."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("127.0.0.1", 0))
        return s.getsockname()[1]

def _worker(local_rank: int, world_size: int, gpus: list,
            master_addr: str, master_port: int,
            extra_run_note: str, resume_path: str = ""):
    rank = local_rank  # single node assumed
    ...
    dist.init_process_group(
        ...
        world_size=world_size,
        rank=rank,
        # a long rank-0-only stretch can push the waiting ranks past the default 600s NCCL
        #   watchdog, so the timeout is raised to 60 minutes
...
def main():
    parser = argparse.ArgumentParser(
        description="MetaCLIP2-L/14@224 + DoRA + FLAIR - DDP entry point (single or multi GPU)")
    ...
    parser.add_argument("--run-note", type=str, default="",
                        help="run directory name (default {timestamp}_%s)" % RUN_NOTE)
    ...
    check_inputs()          # check the inputs before taking a GPU

    run_name = args.run_note or f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{RUN_NOTE}"
    if os.path.exists(f"{RUNS_ROOT}/{run_name}"):    # caught before spawn
        raise SystemExit(f"[error] the run directory already exists: {RUNS_ROOT}/{run_name}\n"
                         f"  to train again under the same name, delete this path first")
    ...
    # check CUDA before anything else
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is not available; DDP training cannot start.")

    gpus = [int(x.strip()) for x in args.gpus.split(",") if x.strip() != ""]
    avail = torch.cuda.device_count()
    for g in gpus:
        if g < 0 or g >= avail:
            raise ValueError(f"GPU id {g} invalid (available: 0..{avail-1})")
    world_size = len(gpus)
    if world_size == 0:
        raise ValueError("--gpus is empty")

    master_port = args.master_port if args.master_port > 0 else _free_port()
    print(f"[launcher] gpus={gpus}  world_size={world_size}  "
          f"master={args.master_addr}:{master_port}")

    if world_size == 1:
        # a single GPU also goes through the DDP path, for one code path only
        _worker(0, 1, gpus, args.master_addr, master_port, run_name, resume_path)
        return

    mp.spawn(
        _worker,
        nprocs=world_size,
        args=(world_size, gpus, args.master_addr, master_port, run_name, resume_path),
        join=True,
    )
