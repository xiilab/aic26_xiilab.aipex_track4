...
"""Copy the selected step checkpoint to the reproducible deployment location.
  ...
  this script  : <run> t* -> assets/model_rep/reranker/internvl_r32

`train.py` writes a LoRA adapter to `<out>/step{N}` every `--save-every` steps; one of those is picked.
...
def main():
    ...
    ap.add_argument("run", help="training run directory (contains step{N}/)")
    ap.add_argument("--step", required=True, help="best step chosen with eval_step.py (e.g. 2500)")
    args = ap.parse_args()

    run = _abs(args.run)
    tag = args.step if str(args.step).startswith("step") else f"step{args.step}"
    src = os.path.join(run, tag)
    problems = []
    if not os.path.isdir(run):
        problems.append(f"path not found: {run}")
    elif not os.path.isdir(src):
        have = sorted(os.path.basename(p) for p in glob.glob(os.path.join(run, "step*")))
        problems.append(f"checkpoint not found: {os.path.relpath(src, _REPO)}\n"
                        f"      pick the best step with:\n"
                        f"      python train/reranker/eval/eval_step.py --member r32 --run {args.run} --steps <list>\n"
    ...
    print(f"         reproduction cache: pipeline/S2_rerank/score_union_hf_4b.py --rep --name {NAME}")
