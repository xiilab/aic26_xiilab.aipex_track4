...
"""Copy the selected step checkpoint to the reproducible deployment location.
  ...
  this script  : <run> t* -> assets/model_rep/reranker/qwen3vl_2b
...
def main():
    ...
    ap.add_argument("--step", required=True, help="best step chosen with eval_step.py (e.g. ex007000 or 7000)")
    args = ap.parse_args()
    ...
    if not os.path.isdir(run):
        ...
        problems.append(f"checkpoint not found: {os.path.relpath(src, _REPO)}\n"
                        ...
                        f"      python train/reranker/eval/eval_step.py --member dora --run {args.run} --steps <list>\n"
    ...
    print(f"         reproduction cache: pipeline/S2_rerank/score_union_qwen_4b.py --rep --name {NAME}")
